...
class SystemVipService
{
    ...
    public function subscribe(User $user, MembershipPackage $package, string $billingCycle): UserSubscription
    {
        ...
        return DB::transaction(function () use ($user, $package, $billingCycle, $price): UserSubscription {
            ...
            $subscription = UserSubscription::query()->create([
                ...
                'status' => 'active',
                'paid_amount' => $price,
                'payment_ref' => 'VIP'.now()->format('ymdHis').Str::upper(Str::random(6)),
                'month_post_count' => 0,
                'month_post_reset_at' => now()->copy()->startOfMonth(),
            ]);

            $this->issueMonthlyVouchersForSubscription($subscription->fresh('membershipPackage'));

            return $subscription->fresh('membershipPackage');
        });
    }
    ...
    public function creditCashbackForCompletedBooking(Booking $booking): float
    {
        ...
        return DB::transaction(function () use ($booking, $customerId, $cashbackAmount, $package): float {
            if (DB::table('user_wallet_ledgers')
