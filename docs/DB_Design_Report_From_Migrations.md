...
- Ngày cập nhật: 2026-06-27 11:03:22 (Asia/Saigon)
- Số file migration đã đọc: 173
- Số bảng trong thiết kế hiện tại: 124
...
| 23 | bookings | Booking | Đơn đặt sân | Quản lý lịch đặt sân, giờ chơi, thanh toán, trạng thái | users (cancelled_by), users (created_by), users (customer_id), venue_courts (venue_court_id), venue_courts (requested_venue_court_id), users (court_changed_by), vouchers (voucher_id), vouchers (venue_voucher_id), vouchers (vip_voucher_id) |
...
| 29 | payments | Payment | Thanh toán | Quản lý giao dịch thanh toán của booking | bookings (booking_id), system_bank_accounts (system_bank_account_id), user_wallets (user_wallet_id), user_wallet_ledgers (user_wallet_ledger_id), user_subscriptions (subscription_id) |
...
| 42 | venue_platform_fee_ledgers | Payment | Công nợ phí nền tảng | Quản lý lịch sử và trạng thái đóng phí nền tảng của cụm sân | platform_fee_tiers (tier_id), venue_clusters (venue_cluster_id), media (payment_proof_media_id), users (payment_confirmed_by), users (payment_rejected_by), internal_receipts (internal_receipt_id), system_bank_accounts (system_bank_account_id) |
...
| 77 | vouchers | Voucher | Mã giảm giá | Lưu voucher hệ thống và voucher sân | users (created_by), user_subscriptions (subscription_id), users (assigned_user_id) |
...
| 99 | venue_location_change_requests | Venue | Yêu cầu đổi địa chỉ sân | Lưu yêu cầu chủ sân gửi để thay đổi địa chỉ, khu vực và map URL của cụm sân. | venue_clusters (venue_cluster_id), users (requested_by), users (reviewed_by) |
| 100 | vn_provinces | System | Quản lý vn provinces | Lưu dữ liệu nghiệp vụ cho bảng `vn_provinces` theo schema migration hiện tại. | Không FK |
| 101 | vn_wards | System | Quản lý vn wards | Lưu dữ liệu nghiệp vụ cho bảng `vn_wards` theo schema migration hiện tại. | vn_provinces (province_code) |
| 102 | violation_types | Moderation | Danh mục loại vi phạm | Danh mục nhóm vi phạm nội dung/hành vi và điểm mặc định dùng trong kiểm duyệt. | Không FK |
| 103 | severity_levels | Moderation | Danh mục mức độ vi phạm | Danh mục cấp độ nghiêm trọng và khoảng điểm dùng để phân loại vi phạm. | Không FK |
| 104 | moderation_thresholds | Moderation | Ngưỡng xử lý kiểm duyệt | Cấu hình ngưỡng cảnh báo và hành động tự động theo chính sách kiểm duyệt. | system_policies (system_policy_id) |
| 105 | violation_records | Moderation | Lịch sử vi phạm | Lưu từng lần ghi nhận vi phạm của đối tượng bị báo cáo hoặc bị hệ thống xử lý. | Không FK |
| 106 | user_lock_logs | Auth/RBAC | Lịch sử khóa tài khoản | Ghi lại lịch sử khóa/mở khóa tài khoản, người thực hiện và lý do. | users (user_id), users (locked_by) |
| 107 | partner_histories | Partner | Lịch sử hồ sơ đối tác | Ghi lại lịch sử thay đổi hồ sơ đối tác để phục vụ audit và theo dõi workflow. | partner_applications (partner_application_id), users (actor_id) |
| 108 | venue_base_prices | Venue | Quản lý venue base prices | Lưu dữ liệu nghiệp vụ cho bảng `venue_base_prices` theo schema migration hiện tại. | venue_clusters (venue_cluster_id), court_types (court_type_id) |
| 109 | venue_unlock_requests | Venue | Quản lý venue unlock requests | Lưu dữ liệu nghiệp vụ cho bảng `venue_unlock_requests` theo schema migration hiện tại. | venue_clusters (venue_cluster_id), users (requested_by), users (reviewed_by) |
| 110 | affiliate_products | System | Quản lý affiliate products | Lưu dữ liệu nghiệp vụ cho bảng `affiliate_products` theo schema migration hiện tại. | venue_clusters (venue_cluster_id) |
| 111 | venue_transfer_requests | Venue | Quản lý venue transfer requests | Lưu dữ liệu nghiệp vụ cho bảng `venue_transfer_requests` theo schema migration hiện tại. | venue_clusters (venue_cluster_id), users (from_owner_id), users (to_owner_id), users (requested_by), users (reviewed_by) |
| 112 | administrative_units | System | Quản lý administrative units | Lưu dữ liệu nghiệp vụ cho bảng `administrative_units` theo schema migration hiện tại. | Không FK |
| 113 | marketplace_listings | System | Quản lý marketplace listings | Lưu dữ liệu nghiệp vụ cho bảng `marketplace_listings` theo schema migration hiện tại. | users (seller_id), court_types (court_type_id), venue_clusters (preferred_venue_cluster_id), users (reviewed_by) |
| 114 | court_membership_tiers | System | Quản lý court membership tiers | Lưu dữ liệu nghiệp vụ cho bảng `court_membership_tiers` theo schema migration hiện tại. | venue_clusters (venue_cluster_id) |
| 115 | court_type_requests | System | Quản lý court type requests | Lưu dữ liệu nghiệp vụ cho bảng `court_type_requests` theo schema migration hiện tại. | court_types (parent_id), users (requested_by) |
| 116 | user_court_memberships | System | Quản lý user court memberships | Lưu dữ liệu nghiệp vụ cho bảng `user_court_memberships` theo schema migration hiện tại. | users (user_id), venue_clusters (venue_cluster_id) |
| 117 | membership_packages | System | Quản lý membership packages | Lưu dữ liệu nghiệp vụ cho bảng `membership_packages` theo schema migration hiện tại. | Không FK |
| 118 | user_subscriptions | System | Quản lý user subscriptions | Lưu dữ liệu nghiệp vụ cho bảng `user_subscriptions` theo schema migration hiện tại. | users (user_id), membership_packages (package_id) |
| 119 | system_wallet_balances | Payment | Quản lý system wallet balances | Lưu dữ liệu nghiệp vụ cho bảng `system_wallet_balances` theo schema migration hiện tại. | system_bank_accounts (system_bank_account_id) |
| 120 | system_wallet_ledgers | Payment | Quản lý system wallet ledgers | Lưu dữ liệu nghiệp vụ cho bảng `system_wallet_ledgers` theo schema migration hiện tại. | system_bank_accounts (system_bank_account_id) |
| 121 | venue_post_comments | Venue | Quản lý venue post comments | Lưu dữ liệu nghiệp vụ cho bảng `venue_post_comments` theo schema migration hiện tại. | venue_posts (venue_post_id), users (user_id), venue_post_comments (parent_id) |
| 122 | user_court_membership_histories | System | Quản lý user court membership histories | Lưu dữ liệu nghiệp vụ cho bảng `user_court_membership_histories` theo schema migration hiện tại. | user_court_memberships (membership_id), users (user_id), venue_clusters (venue_cluster_id) |
| 123 | document_signing_requests | Document | Quản lý document signing requests | Lưu dữ liệu nghiệp vụ cho bảng `document_signing_requests` theo schema migration hiện tại. | generated_documents (generated_document_id), verification_codes (verification_code_id), users (user_id), generated_document_signatures (signed_signature_id) |
| 124 | platform_fee_email_logs | System | Quản lý platform fee email logs | Lưu dữ liệu nghiệp vụ cho bảng `platform_fee_email_logs` theo schema migration hiện tại. | venue_platform_fee_ledgers (ledger_id), venue_clusters (venue_cluster_id), users (triggered_by) |
...
| 8 | province_code | string(20) | Có | - | - | Ma tinh/thanh pho tu administrative_units. | CODE-001 |
| 9 | district_code | string(20) | Có | - | - | Ma quan/huyen tu administrative_units. | CODE-001 |
| 10 | ward | string(255) | Có | - | - | Xã/Phường | example |
| 11 | ward_code | string(20) | Có | - | - | Ma phuong/xa tu administrative_units. | CODE-001 |
| 12 | address | text | Không | - | - | Địa chỉ sân để hiển thị và mở Google Maps. | example |
| 13 | map_url | string(1000) | Có | - | - | Link Google Maps lưu lại từ form đăng ký/quản lý sân. | https://sportgo.vn/example |
| 14 | latitude | decimal(10,7) | Không | - | - | Vĩ độ dùng để tìm sân gần vị trí hiện tại. | 1 |
| 15 | longitude | decimal(10,7) | Không | - | - | Kinh độ dùng để tìm sân gần vị trí hiện tại. | 1 |
| 16 | layout_decorations | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 17 | amenities | json | Có | - | - | JSON danh sách tiện ích như bãi xe, đèn, phòng tắm. | {"key":"value"} |
| 18 | status | enum(pending, active, locked) | Không | pending | INDEX | Trạng thái cụm: pending chờ duyệt, active hoạt động, locked bị khóa. | pending |
| 19 | status_reason | text | Có | - | - | Lý do khóa cụm sân để chủ sân biết. | active |
| 20 | locked_at | timestamp | Có | - | - | Thời điểm cụm sân bị khóa. | 2026-06-22 09:00:00 |
| 21 | locked_until | timestamp | Có | - | INDEX | Thời điểm hết khóa tạm thời của cụm sân. | 2026-06-22 09:00:00 |
| 22 | locked_by | char(36) | Có | - | FK | Admin/nhân viên khóa cụm sân. | 10000000-0000-0000-0000-000000000001 |
| 23 | rating_avg | decimal(3,2) | Không | 0 | INDEX | Điểm trung bình sân, tính từ reviews. | 1 |
| 24 | rating_count | unsignedInteger | Không | 0 | - | Số lượt review sân. | 1 |
| 25 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 26 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
| 4 | min_advance_booking_minutes | unsignedInteger | Không | 30 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 1 |
| 5 | fixed_open_time | time | Không | 08:00:00 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 08:00:00 |
| 6 | fixed_close_time | time | Không | 22:00:00 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 08:00:00 |
| 7 | weekly_operating_hours | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 8 | special_operating_hours | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 9 | slot_hold_minutes | unsignedInteger | Không | 20 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 1 |
| 10 | reminder_before_minutes | unsignedInteger | Không | 30 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 1 |
| 11 | allow_full_payment | boolean | Không | true | - | Cờ bật/tắt cho hành vi hoặc trạng thái tương ứng. | true |
| 12 | allow_deposit | boolean | Không | true | - | Cờ bật/tắt cho hành vi hoặc trạng thái tương ứng. | true |
| 13 | allow_no_prepay | boolean | Không | true | - | Cờ bật/tắt cho hành vi hoặc trạng thái tương ứng. | true |
| 14 | auto_approve_full_payment | boolean | Không | false | - | Cờ bật/tắt cho hành vi hoặc trạng thái tương ứng. | true |
| 15 | deposit_percent | decimal(5,2) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 1 |
| 16 | cancel_before_hours | unsignedInteger | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 1 |
| 17 | refund_percent | unsignedInteger | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_configs`. | 1 |
| 18 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 19 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
```json
{
    ...
    "fixed_open_time": "08:00:00",
    "fixed_close_time": "08:00:00",
...
| 9 | evidence_image | string(500) | Có | - | - | Đường dẫn ảnh minh chứng do chủ sân gửi kèm | example |
| 10 | approved_venue_court_id | char(36) | Có | - | INDEX | ID tham chiếu đối tượng liên quan trong nghiệp vụ. | 10000000-0000-0000-0000-000000000001 |
...
| 25 | venue_province_code | string(20) | Có | - | - | Ma tinh/thanh pho cua cum san dang ky. | CODE-001 |
| 26 | venue_district | string(100) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | example |
| 27 | venue_district_code | string(20) | Có | - | - | Ma quan/huyen cua cum san dang ky. | CODE-001 |
| 28 | venue_ward | string(100) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | example |
| 29 | venue_ward_code | string(20) | Có | - | - | Ma phuong/xa cua cum san dang ky. | CODE-001 |
| 30 | venue_map_url | string(1000) | Có | - | - | Link Google Maps user dán trong form. | https://sportgo.vn/example |
| 31 | venue_latitude | decimal(10,7) | Không | - | - | Vĩ độ cụm sân. | 1 |
| 32 | venue_longitude | decimal(10,7) | Không | - | - | Kinh độ cụm sân. | 1 |
| 33 | venue_phone | string(30) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | 0901234567 |
| 34 | venue_email | string(255) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | user@sportgo.vn |
| 35 | venue_description | text | Có | - | - | Mô tả hoặc ghi chú bổ sung cho bản ghi. | Nội dung mẫu |
| 36 | expected_opening_hours | string(255) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | example |
| 37 | parking_info | text | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | example |
| 38 | amenities | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 39 | court_count_total | unsignedInteger | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 40 | base_price_per_hour | unsignedInteger | Không | 0 | - | Gia co ban/gio cua cum san khi dang ky doi tac. | 100000 |
| 41 | bank_name | string(150) | Có | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 42 | bank_code | string(50) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | CODE-001 |
| 43 | account_number | string(50) | Có | - | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 44 | account_holder_name | string(255) | Có | - | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 45 | bank_branch | string(255) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `partner_applications`. | example |
| 46 | bank_verification_status | enum(pending, verified, rejected) | Không | pending | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | pending |
| 47 | bank_verified_at | timestamp | Có | - | - | Thoi diem tai khoan ngan hang duoc xac minh tu dong. | 2026-06-22 09:00:00 |
| 48 | status | enum(pending, reviewing, approved, rejected, cancelled, draft, submitted, need_supplement, approved_pending_contract, contract_pending_owner_signature, contract_pending_sportgo_signature, completed) | Không | submitted | INDEX | Trạng thái hồ sơ. | pending |
| 49 | reviewed_by | char(36) | Có | - | FK | Admin/nhân viên duyệt hồ sơ. | 10000000-0000-0000-0000-000000000001 |
| 50 | status_reason | text | Có | - | - | Lý do từ chối/hủy. | active |
| 51 | approved_venue_cluster_id | char(36) | Có | - | INDEX | ID cụm sân được tạo sau khi duyệt; logical. | 10000000-0000-0000-0000-000000000001 |
| 52 | current_contract_id | char(36) | Có | - | INDEX | ID tham chiếu đối tượng liên quan trong nghiệp vụ. | 10000000-0000-0000-0000-000000000001 |
| 53 | submitted_at | timestamp | Có | CURRENT_TIMESTAMP | INDEX | Thoi diem user gui ho so sau khi da ky don. | 2026-06-22 09:00:00 |
| 54 | reviewed_at | timestamp | Có | - | - | Thời điểm admin xử lý hồ sơ. | 2026-06-22 09:00:00 |
| 55 | terminated_at | timestamp | Có | - | - | Thời điểm chấm dứt hợp đồng hợp tác | 2026-06-22 09:00:00 |
| 56 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 57 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
| 14 | membership_tier_discount_amount | decimal(12,2) | Không | 0 | - | Số tiền giảm giá từ hạng thành viên | 100000 |
| 15 | membership_tier | enum(standard, silver, gold, diamond) | Có | - | - | Hạng thành viên áp dụng tại thời điểm đặt booking | example |
| 16 | cashback_amount | decimal(12,2) | Không | 0 | - | Hoàn tiền VIP đã hoàn vào ví sau booking | 100000 |
| 17 | system_discount_amount | decimal(12,2) | Không | 0 | - | Phần giảm do nền tảng chịu. | 100000 |
| 18 | venue_discount_amount | decimal(12,2) | Không | 0 | - | Phần giảm do chủ sân/cụm sân chịu. | 100000 |
| 19 | final_amount | decimal(12,2) | Có | - | - | Số tiền cuối cùng sau voucher/discount. | 100000 |
| 20 | voucher_id | char(36) | Có | - | FK, INDEX | Voucher chính áp dụng nếu chỉ cho một voucher/booking. | 10000000-0000-0000-0000-000000000001 |
| 21 | voucher_code_snapshot | string(50) | Có | - | - | Snapshot mã voucher tại thời điểm đặt. | CODE-001 |
| 22 | venue_voucher_id | char(36) | Có | - | FK, INDEX | Khóa ngoại tham chiếu bảng `vouchers`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 23 | venue_voucher_code_snapshot | string(50) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `bookings`. | CODE-001 |
| 24 | vip_voucher_id | char(36) | Có | - | FK, INDEX | Khóa ngoại tham chiếu bảng `vouchers`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 25 | vip_voucher_code_snapshot | string(50) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `bookings`. | CODE-001 |
| 26 | payment_option | enum(full_payment, deposit, no_prepay) | Không | no_prepay | - | Kiểu thanh toán user chọn. | example |
| 27 | required_payment_amount | decimal(12,2) | Không | 0 | - | Số tiền tối thiểu cần thanh toán. | 100000 |
| 28 | source | enum(online, counter) | Không | online | - | Nguồn booking: online hoặc counter. | example |
| 29 | booking_type | enum(single, recurring) | Không | single | - | Kiểu booking: single=đặt lẻ; recurring=đặt cố định. | single |
| 30 | recurring_group_code | string(30) | Có | - | - | Mã nhóm đơn đặt cố định. | CODE-001 |
| 31 | recurring_start_date | date | Có | - | - | Ngày bắt đầu của rule đặt cố định. | 2026-06-22 |
| 32 | recurring_end_date | date | Có | - | - | Ngày kết thúc của rule đặt cố định. | 2026-06-22 |
| 33 | recurrence_type | enum(daily, weekly, monthly) | Có | - | - | Kiểu lặp của đơn cố định. | daily |
| 34 | recurrence_interval | unsignedInteger | Có | - | - | Khoảng lặp theo recurrence_type. | 1 |
| 35 | recurrence_days_of_week | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 36 | recurrence_days_of_month | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 37 | status | enum(pending_approval, pending_payment, confirmed, checked_in, completed, cancelled, expired, rejected) | Không | pending_approval | INDEX | Trạng thái booking. | pending_approval |
| 38 | walk_in_name | string(255) | Có | - | - | Tên khách tại quầy khi customer_id null. | Ví dụ SportGo |
| 39 | walk_in_phone | string(20) | Có | - | - | Số điện thoại khách tại quầy. | 0901234567 |
| 40 | status_reason | text | Có | - | - | Lý do hủy/từ chối/hết hiệu lực booking. | active |
| 41 | cancelled_by | char(36) | Có | - | FK | User/admin/chủ sân thực hiện hủy booking. | 10000000-0000-0000-0000-000000000001 |
| 42 | cancellation_initiator | enum(customer, owner, system, admin) | Có | - | - | Ben khoi tao huy booking | example |
| 43 | cancellation_reason_type | enum(customer_request, owner_maintenance, owner_emergency, venue_locked, system_auto, admin_action, no_payment, expired) | Có | - | - | Phan loai ly do huy de tinh chinh sach hoan tien | Nội dung mẫu |
| 44 | cancelled_at | timestamp | Có | - | - | Thời điểm booking bị hủy. | 2026-06-22 09:00:00 |
| 45 | created_by | char(36) | Có | - | FK | Người tạo booking. | 10000000-0000-0000-0000-000000000001 |
| 46 | court_changed_by | char(36) | Có | - | FK | Chủ sân/nhân viên đổi sân. | 10000000-0000-0000-0000-000000000001 |
| 47 | court_changed_at | timestamp | Có | - | - | Thời điểm đổi sân con. | 2026-06-22 09:00:00 |
| 48 | court_changed_reason | text | Có | - | - | Lý do đổi sân. | Nội dung mẫu |
| 49 | reminder_sent_at | timestamp | Có | - | - | Thời điểm hệ thống đã gửi nhắc lịch. | 2026-06-22 09:00:00 |
| 50 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 51 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- FK: venue_voucher_id -> vouchers.id (on delete: set null)
- FK: vip_voucher_id -> vouchers.id (on delete: set null)
...
- INDEX: bookings_cancellation_reason_index (cancellation_initiator, cancellation_reason_type)
- INDEX: bookings_venue_voucher_id_index (venue_voucher_id)
- INDEX: bookings_vip_voucher_id_index (vip_voucher_id)
...
- bookings n-1 vouchers qua venue_voucher_id.
- bookings n-1 vouchers qua vip_voucher_id.
...
| 3 | is_vip_priority | boolean | Không | false | - | Khiếu nại từ user VIP, ưu tiên xử lý | true |
| 4 | booking_id | char(36) | Có | - | FK | Booking liên quan nếu khiếu nại phát sinh từ booking. | 10000000-0000-0000-0000-000000000001 |
| 5 | venue_cluster_id | char(36) | Có | - | FK | Cụm sân liên quan nếu là khiếu nại với sân. | 10000000-0000-0000-0000-000000000001 |
| 6 | customer_id | char(36) | Không | - | FK | User gửi khiếu nại. | 10000000-0000-0000-0000-000000000001 |
| 7 | content | text | Không | - | - | Nội dung khiếu nại. | Nội dung mẫu |
| 8 | status | enum(open, processing, resolved, rejected, closed) | Không | open | INDEX | Trạng thái khiếu nại. | open |
| 9 | assigned_to | char(36) | Có | - | FK | Nhân viên/admin được gán xử lý khiếu nại. | 10000000-0000-0000-0000-000000000001 |
| 10 | resolve_note | text | Có | - | - | Ghi chú giải quyết khiếu nại. | Nội dung mẫu |
| 11 | status_reason | text | Có | - | - | Lý do/ghi chú giải thích cho trạng thái hoặc quyết định xử lý. | active |
| 12 | resolved_by | char(36) | Có | - | FK | Người xử lý xong khiếu nại. | 10000000-0000-0000-0000-000000000001 |
| 13 | resolved_at | timestamp | Có | - | - | Thời điểm xử lý xong. | 2026-06-22 09:00:00 |
...
| 4 | title | string(255) | Có | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 5 | content | longText | Không | - | - | Nội dung chính của bản ghi. | Nội dung mẫu |
| 6 | short_description | string(500) | Không | - | - | Mô tả hoặc ghi chú bổ sung cho bản ghi. | Nội dung mẫu |
| 7 | meta_title | string(255) | Có | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 8 | meta_description | string(500) | Có | - | - | Mô tả hoặc ghi chú bổ sung cho bản ghi. | Nội dung mẫu |
| 9 | status | enum(pending_review, published, rejected, hidden) | Không | pending_review | INDEX | Trạng thái xử lý hoặc vòng đời của bản ghi. | pending_review |
...
| 11 | reviewed_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `reviewed_at`. | 2026-06-22 09:00:00 |
| 12 | status_reason | text | Có | - | - | Lý do/ghi chú giải thích cho trạng thái hoặc quyết định xử lý. | active |
| 13 | view_count | unsignedBigInteger | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 14 | like_count | unsignedInteger | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 15 | comment_count | unsignedInteger | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
...
| 18 | post_type | enum(promotion, tournament, news, notice, recruitment) | Không | news | - | Loại hoặc nhóm phân loại của bản ghi. | promotion |
| 19 | valid_from | timestamp | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `venue_posts`. | 2026-06-22 09:00:00 |
| 20 | valid_to | timestamp | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `venue_posts`. | 2026-06-22 09:00:00 |
| 21 | slug | string(255) | Có | - | UNIQUE | Chuỗi định danh thân thiện URL và SEO. | example |
...
- UNIQUE: venue_posts_slug_unique (slug)
...
```json
{
    ...
    "title": "Ví dụ SportGo",
    "content": "Nội dung mẫu",
    "short_description": "Nội dung mẫu",
    "meta_title": "Ví dụ SportGo",
    "meta_description": "Nội dung mẫu"
}
```
...
| 3 | payment_context | enum(booking, vip_subscription) | Không | booking | - | Nghiep vu thanh toan. | example |
| 4 | booking_id | char(36) | Có | - | FK | Booking được thanh toán. | 10000000-0000-0000-0000-000000000001 |
| 5 | subscription_id | char(36) | Có | - | FK | Subscription VIP duoc thanh toan neu payment_context=vip_subscription. | 10000000-0000-0000-0000-000000000001 |
| 6 | system_bank_account_id | char(36) | Có | - | FK, INDEX | Tài khoản hệ thống nhận tiền cho payment này. | 10000000-0000-0000-0000-000000000001 |
| 7 | user_wallet_id | char(36) | Có | - | FK, INDEX | Ví user dùng trong payment nếu thanh toán bằng ví hoặc mixed. | 10000000-0000-0000-0000-000000000001 |
| 8 | user_wallet_ledger_id | char(36) | Có | - | FK, INDEX | Ledger debit ví user liên quan payment này. | 10000000-0000-0000-0000-000000000001 |
| 9 | amount | decimal(12,2) | Không | - | - | Số tiền của lần thanh toán này. | 100000 |
| 10 | wallet_amount | decimal(12,2) | Không | 0 | - | Phần tiền thanh toán bằng ví user. | 100000 |
| 11 | gateway_amount | decimal(12,2) | Không | 0 | - | Phần tiền thanh toán qua gateway/chuyển khoản. | 100000 |
| 12 | payment_kind | enum(full, deposit, partial) | Không | partial | - | Loại thanh toán. | example |
| 13 | method | enum(50) | Không | sepay | INDEX | Phương thức thanh toán/ghi nhận tiền. | example |
| 14 | gateway_txn_id | string(100) | Có | - | UNIQUE | Mã giao dịch từ cổng thanh toán. | 1 |
| 15 | gateway_response | json | Có | - | - | JSON phản hồi từ gateway. | {"key":"value"} |
| 16 | status | enum(pending, paid, failed, refunded) | Không | pending | INDEX | Trạng thái payment. | pending |
| 17 | paid_at | timestamp | Có | - | INDEX | Thời điểm thanh toán thành công. | 2026-06-22 09:00:00 |
| 18 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 19 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- FK: subscription_id -> user_subscriptions.id (on delete: set null)
...
- INDEX: payments_context_status_index (payment_context, status)
- INDEX: payments_subscription_status_index (subscription_id, status)
...
- payments n-1 user_subscriptions qua subscription_id.
...
| 3 | type | string(100) | Không | - | INDEX | Loai thong bao. | default |
...
| 14 | system_bank_account_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `system_bank_accounts`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 15 | payment_code | string(50) | Có | - | UNIQUE | Trường dữ liệu phục vụ nghiệp vụ của bảng `venue_platform_fee_ledgers`. | CODE-001 |
| 16 | gateway_txn_id | string(100) | Có | - | UNIQUE | ID tham chiếu đối tượng liên quan trong nghiệp vụ. | 1 |
| 17 | gateway_response | json | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 18 | payment_proof_media_id | char(36) | Có | - | FK | File bằng chứng thanh toán gần nhất trong media. | 10000000-0000-0000-0000-000000000001 |
| 19 | payment_proof_status | enum(none, submitted, approved, rejected) | Không | none | INDEX | Trạng thái duyệt bằng chứng thanh toán. | none |
| 20 | payment_proof_note | text | Có | - | - | Ghi chú từ owner/admin về bằng chứng thanh toán. | Nội dung mẫu |
| 21 | status | enum(pending, paid, overdue, cancelled) | Không | pending | INDEX | Trạng thái xử lý hoặc vòng đời của bản ghi. | pending |
| 22 | paid_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `paid_at`. | 2026-06-22 09:00:00 |
| 23 | payment_confirmed_by | char(36) | Có | - | FK | Admin xác nhận thanh toán. | 10000000-0000-0000-0000-000000000001 |
| 24 | payment_confirmed_at | timestamp | Có | - | - | Thời điểm xác nhận thanh toán. | 2026-06-22 09:00:00 |
| 25 | payment_rejected_by | char(36) | Có | - | FK | Admin từ chối bằng chứng thanh toán. | 10000000-0000-0000-0000-000000000001 |
| 26 | payment_rejected_at | timestamp | Có | - | - | Thời điểm từ chối bằng chứng. | 2026-06-22 09:00:00 |
| 27 | payment_reject_reason | text | Có | - | - | Lý do từ chối bằng chứng thanh toán. | Nội dung mẫu |
| 28 | locked_venue_at | timestamp | Có | - | - | Thời điểm hệ thống/admin khóa cụm sân vì quá hạn phí. | 2026-06-22 09:00:00 |
| 29 | internal_receipt_id | char(36) | Có | - | FK | Phiếu/hóa đơn nội bộ phát hành cho kỳ phí. | 10000000-0000-0000-0000-000000000001 |
| 30 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 31 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- UNIQUE: venue_platform_fee_ledgers_payment_code_unique (payment_code)
- UNIQUE: venue_platform_fee_ledgers_gateway_txn_id_unique (gateway_txn_id)
...
- venue_platform_fee_ledgers n-1 system_bank_accounts qua system_bank_account_id.
...
| 6 | refund_destination | enum(original_payment, user_wallet, bank_account, cash) | Không | user_wallet | INDEX | Đích hoàn tiền: payment gốc, ví user hoặc tài khoản ngân hàng. | example |
...
| 15 | status | enum(pending_confirmation, processing, completed, completed_cash, failed, rejected, pending_owner_confirmation, owner_confirmed, owner_rejected, admin_processing, cancelled) | Không | pending_owner_confirmation | - | Trạng thái refund. | pending_confirmation |
...
| 25 | cash_refunded_by | char(36) | Có | - | - | Người dùng/tác nhân thực hiện thao tác tương ứng. | 10000000-0000-0000-0000-000000000001 |
| 26 | cash_refunded_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `cash_refunded_at`. | 2026-06-22 09:00:00 |
| 27 | cash_refund_note | text | Có | - | - | Mô tả hoặc ghi chú bổ sung cho bản ghi. | Nội dung mẫu |
| 28 | gateway_refund_txn_id | string(100) | Có | - | INDEX | Mã giao dịch hoàn tiền từ gateway nếu có. | 1 |
| 29 | payout_transfer_code | string(40) | Có | - | UNIQUE | Mã nội dung chuyển khoản admin dùng khi hoàn tiền bằng QR. | CODE-001 |
| 30 | payout_qr_created_at | timestamp | Có | - | - | Thời điểm tạo QR chuyển tiền hoàn tiền gần nhất. | 2026-06-22 09:00:00 |
| 31 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 32 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
| 13 | notified_booking_ids | json | Có | - | - | Danh sach booking_id da duoc thong bao ve lock dot xuat | {"key":"value"} |
| 14 | notification_sent_at | timestamp | Có | - | - | Thoi diem da gui thong bao cho booking bi anh huong | 2026-06-22 09:00:00 |
| 15 | expires_at | timestamp | Không | - | INDEX | Thời điểm lock hết hạn. | 2026-06-22 09:00:00 |
| 16 | created_at | timestamp | Có | - | - | Thời điểm tạo lock. | 2026-06-22 09:00:00 |
...
| 4 | type | string(150) | Không | - | INDEX | Muc dich ma OTP, ho tro type kem nonce cho ky van ban. | register |
...
| 10 | status | string(40) | Không | active | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | active |
| 11 | status_reason | text | Có | - | - | Lý do/ghi chú giải thích cho trạng thái hoặc quyết định xử lý. | active |
| 12 | cancelled_by | char(36) | Có | - | - | Người dùng/tác nhân thực hiện thao tác tương ứng. | 10000000-0000-0000-0000-000000000001 |
| 13 | cancelled_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `cancelled_at`. | 2026-06-22 09:00:00 |
| 14 | maintenance_lock_id | char(36) | Có | - | INDEX | ID tham chiếu đối tượng liên quan trong nghiệp vụ. | 10000000-0000-0000-0000-000000000001 |
| 15 | interrupted_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `interrupted_at`. | 2026-06-22 09:00:00 |
| 16 | played_minutes | unsignedInteger | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_items`. | 1 |
| 17 | remaining_minutes | unsignedInteger | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_items`. | 1 |
| 18 | incident_refund_ratio | decimal(8,6) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_items`. | 1 |
| 19 | incident_resolution | string(40) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `booking_items`. | example |
| 20 | incident_original_court_id | char(36) | Có | - | - | ID tham chiếu đối tượng liên quan trong nghiệp vụ. | 10000000-0000-0000-0000-000000000001 |
| 21 | court_changed_by | char(36) | Có | - | FK | Người đổi sân. | 10000000-0000-0000-0000-000000000001 |
| 22 | court_changed_at | timestamp | Có | - | - | Thời điểm đổi sân. | 2026-06-22 09:00:00 |
| 23 | court_changed_reason | text | Có | - | - | Lý do đổi sân. | Nội dung mẫu |
| 24 | sort_order | unsignedInteger | Không | 0 | - | Thứ tự hiển thị. | 1 |
| 25 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 26 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- INDEX: booking_items_status_court_time_index (status, venue_court_id, start_time)
- INDEX: booking_items_maintenance_lock_id_index (maintenance_lock_id)
...
| 10 | payment_method | string(30) | Có | - | - | cash hoặc bank_transfer khi admin đã chi trả. | example |
| 11 | transfer_reference | string(100) | Có | - | - | Mã giao dịch khi chi trả bằng chuyển khoản. | example |
| 12 | paid_note | text | Có | - | - | Ghi chú chi trả rút tiền người dùng. | Nội dung mẫu |
| 13 | payout_transfer_code | string(30) | Có | - | UNIQUE | Nội dung chuyển khoản dùng để đối soát SePay. | CODE-001 |
| 14 | payout_qr_created_at | timestamp | Có | - | - | Thời điểm tạo QR chi trả. | 2026-06-22 09:00:00 |
| 15 | requested_at | timestamp | Không | CURRENT_TIMESTAMP | - | Thời điểm user gửi yêu cầu. | 2026-06-22 09:00:00 |
| 16 | approved_at | timestamp | Có | - | - | Thời điểm duyệt. | 2026-06-22 09:00:00 |
| 17 | paid_at | timestamp | Có | - | - | Thời điểm chi trả. | 2026-06-22 09:00:00 |
| 18 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 19 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- UNIQUE: user_withdrawal_requests_payout_transfer_code_unique (payout_transfer_code)
...
| 19 | source | enum(manual, vip_subscription) | Không | manual | - | Nguồn phát hành voucher | example |
| 20 | subscription_id | char(36) | Có | - | FK, INDEX | FK den user_subscriptions neu source=vip_subscription | 10000000-0000-0000-0000-000000000001 |
| 21 | assigned_user_id | char(36) | Có | - | FK, INDEX | User được gán riêng voucher VIP nếu có. | 10000000-0000-0000-0000-000000000001 |
| 22 | created_by | char(36) | Có | - | FK | Admin/owner tạo voucher. | 10000000-0000-0000-0000-000000000001 |
| 23 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 24 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: created_by -> users.id (on delete: set null)
- FK: subscription_id -> user_subscriptions.id (on delete: set null)
- FK: assigned_user_id -> users.id (on delete: set null)
...
- INDEX: vouchers_subscription_id_index (subscription_id)
- INDEX: vouchers_assigned_user_index (assigned_user_id)

### 4. Quan hệ với bảng khác
- vouchers n-1 users qua created_by.
- vouchers n-1 user_subscriptions qua subscription_id.
- vouchers n-1 users qua assigned_user_id.
...
| 3 | scope_type | enum(all, venue_cluster, court_type, booking_type, membership_tier, vip_package) | Không | - | - | Loại phạm vi áp dụng. | all |
...
| 6 | document_version | unsignedInteger | Không | 1 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `generated_documents`. | 1 |
...
| 9 | entity_type | string(100) | Có | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 10 | entity_id | string(100) | Có | - | - | ID tham chiếu đối tượng liên quan trong nghiệp vụ. | 1 |
| 11 | partner_application_id | char(36) | Có | - | FK, INDEX | Khóa ngoại tham chiếu bảng `partner_applications`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 12 | partner_contract_id | char(36) | Có | - | FK, INDEX | Khóa ngoại tham chiếu bảng `partner_contracts`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 13 | partner_termination_request_id | char(36) | Có | - | FK, INDEX | Khóa ngoại tham chiếu bảng `partner_termination_requests`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 14 | partner_settlement_id | char(36) | Có | - | FK, INDEX | Khóa ngoại tham chiếu bảng `partner_settlements`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 15 | owner_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 16 | venue_cluster_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `venue_clusters`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 17 | title | string(255) | Có | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 18 | status | string(80) | Không | generated | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | draft |
| 19 | render_data | json | Không | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | {"key":"value"} |
| 20 | generated_file_media_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `media`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 21 | signed_file_media_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `media`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 22 | final_file_media_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `media`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 23 | generated_file_path | string(1000) | Có | - | - | Đường dẫn hoặc URL tới tài nguyên liên quan. | /storage/example.pdf |
| 24 | final_file_path | string(1000) | Có | - | - | Đường dẫn hoặc URL tới tài nguyên liên quan. | /storage/example.pdf |
| 25 | file_hash | string(128) | Có | - | - | Đường dẫn hoặc URL tới tài nguyên liên quan. | example |
| 26 | generated_by | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 27 | generated_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `generated_at`. | 2026-06-22 09:00:00 |
| 28 | locked_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `locked_at`. | 2026-06-22 09:00:00 |
| 29 | completed_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `completed_at`. | 2026-06-22 09:00:00 |
| 30 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 31 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
## Tên bảng: vn_provinces

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `vn_provinces` theo schema migration hiện tại.

### 2. Danh sách trường

| STT | Tên trường | Kiểu dữ liệu | Null | Default | Khóa/Ràng buộc | Mô tả tác dụng trường | Ví dụ |
|---|---|---|---|---|---|---|---|
| 1 | code | string(255) | Không | - | PK | Mã định danh nghiệp vụ dễ đọc. | 1 |
| 2 | name | string(255) | Không | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 3 | codename | string(255) | Có | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 4 | division_type | string(255) | Có | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 5 | phone_code | integer | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `vn_provinces`. | 0901234567 |
| 6 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 7 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `vn_wards` theo schema migration hiện tại.
...
| 3 | codename | string(255) | Có | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 4 | division_type | string(255) | Có | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 5 | province_code | string(255) | Không | - | FK | Khóa ngoại tham chiếu bảng `vn_provinces`, dùng để liên kết bản ghi với dữ liệu liên quan. | CODE-001 |
| 6 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 7 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: code
- FK: province_code -> vn_provinces.code (on delete: cascade)

### 4. Quan hệ với bảng khác
- vn_wards n-1 vn_provinces qua province_code.
...
## Tên bảng: venue_base_prices

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `venue_base_prices` theo schema migration hiện tại.
...
| 2 | venue_cluster_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `venue_clusters`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 3 | court_type_id | unsignedBigInteger | Không | - | FK | Khóa ngoại tham chiếu bảng `court_types`, dùng để liên kết bản ghi với dữ liệu liên quan. | 1 |
| 4 | price | decimal(12,2) | Không | 10000 | - | Giá trị tiền tệ dùng trong tính toán và đối soát. | 100000 |
| 5 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 6 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- FK: venue_cluster_id -> venue_clusters.id (on delete: cascade)
- FK: court_type_id -> court_types.id (on delete: restrict)
- UNIQUE: venue_base_prices_cluster_court_unique (venue_cluster_id, court_type_id)

### 4. Quan hệ với bảng khác
- venue_base_prices n-1 venue_clusters qua venue_cluster_id.
- venue_base_prices n-1 court_types qua court_type_id.
...
## Tên bảng: venue_unlock_requests

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `venue_unlock_requests` theo schema migration hiện tại.
...
| 3 | requested_by | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 4 | reviewed_by | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 5 | status | string(20) | Không | pending | - | pending, approved, rejected | active |
| 6 | reason | text | Không | - | - | Lý do giải trình của chủ sân. | Nội dung mẫu |
| 7 | admin_note | text | Có | - | - | Phản hồi của admin. | Nội dung mẫu |
| 8 | reviewed_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `reviewed_at`. | 2026-06-22 09:00:00 |
| 9 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 10 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- FK: requested_by -> users.id (on delete: cascade)
- FK: reviewed_by -> users.id (on delete: set null)
- INDEX: venue_unlock_requests_venue_cluster_id_status_index (venue_cluster_id, status)

### 4. Quan hệ với bảng khác
- venue_unlock_requests n-1 venue_clusters qua venue_cluster_id.
- venue_unlock_requests n-1 users qua requested_by.
- venue_unlock_requests n-1 users qua reviewed_by.
...
## Tên bảng: affiliate_products

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `affiliate_products` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | venue_cluster_id | char(36) | Không | - | FK, INDEX | ID của cụm sân chứa sản phẩm này. | 10000000-0000-0000-0000-000000000001 |
| 3 | name | string(255) | Không | - | - | Tên sản phẩm tiếp thị liên kết. | Ví dụ SportGo |
| 4 | description | text | Có | - | - | Mô tả sản phẩm. | Nội dung mẫu |
| 5 | image_path | string(255) | Có | - | - | Đường dẫn ảnh sản phẩm. | /storage/example.pdf |
| 6 | price | decimal(15,2) | Có | - | - | Giá bán hiển thị (VND). | 100000 |
| 7 | original_price | decimal(15,2) | Có | - | - | Giá gốc/giá so sánh để hiển thị giảm giá. | 100000 |
| 8 | affiliate_url | text | Không | - | - | Đường dẫn link tiếp thị liên kết Shopee, Lazada... | https://sportgo.vn/example |
| 9 | platform_name | string(100) | Có | - | - | Tên nền tảng (Shopee, Lazada, Tiki, TikTok Shop...). | Ví dụ SportGo |
| 10 | is_active | boolean | Không | true | INDEX | Trạng thái kích hoạt ẩn/hiện sản phẩm. | true |
| 11 | click_count | unsignedInteger | Không | 0 | - | Số lượng click chuyển tiếp của sản phẩm. | 1 |
| 12 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 13 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- PK: id
- FK: venue_cluster_id -> venue_clusters.id (on delete: cascade)
- INDEX: affiliate_products_venue_cluster_id_index (venue_cluster_id)
- INDEX: affiliate_products_is_active_index (is_active)

### 4. Quan hệ với bảng khác
- affiliate_products n-1 venue_clusters qua venue_cluster_id.
...
```json
{
    ...
    "affiliate_url": "https://sportgo.vn/example"
}
```
...
## Tên bảng: venue_transfer_requests

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `venue_transfer_requests` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | transfer_code | string(50) | Không | - | UNIQUE | Ma yeu cau chuyen giao cum san. | CODE-001 |
| 3 | venue_cluster_id | char(36) | Không | - | FK | Cum san duoc chuyen giao. | 10000000-0000-0000-0000-000000000001 |
| 4 | from_owner_id | char(36) | Không | - | FK | Chu san hien tai. | 10000000-0000-0000-0000-000000000001 |
| 5 | to_owner_id | char(36) | Không | - | FK | Chu san tiep nhan. | 10000000-0000-0000-0000-000000000001 |
| 6 | reason | text | Có | - | - | Ly do chuyen giao. | Nội dung mẫu |
| 7 | status | enum(pending, reviewing, approved, rejected, completed, cancelled) | Không | pending | - | Trang thai yeu cau chuyen giao. | pending |
| 8 | requested_by | char(36) | Không | - | FK | Nguoi tao yeu cau. | 10000000-0000-0000-0000-000000000001 |
| 9 | reviewed_by | char(36) | Có | - | FK | Admin duyet yeu cau. | 10000000-0000-0000-0000-000000000001 |
| 10 | status_reason | text | Có | - | - | Ly do tu choi hoac huy. | active |
| 11 | effective_date | date | Có | - | - | Ngay chuyen giao co hieu luc. | 2026-06-22 |
| 12 | completed_at | timestamp | Có | - | - | Thoi diem hoan tat chuyen giao. | 2026-06-22 09:00:00 |
| 13 | reviewed_at | timestamp | Có | - | - | Thoi diem admin xu ly. | 2026-06-22 09:00:00 |
| 14 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 15 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- PK: id
- FK: venue_cluster_id -> venue_clusters.id (on delete: restrict)
- FK: from_owner_id -> users.id (on delete: restrict)
- FK: to_owner_id -> users.id (on delete: restrict)
- FK: requested_by -> users.id (on delete: restrict)
- FK: reviewed_by -> users.id (on delete: set null)
- UNIQUE: venue_transfer_requests_transfer_code_unique (transfer_code)
- INDEX: venue_transfer_requests_cluster_status_index (venue_cluster_id, status)
- INDEX: venue_transfer_requests_from_owner_status_index (from_owner_id, status)
- INDEX: venue_transfer_requests_to_owner_status_index (to_owner_id, status)

### 4. Quan hệ với bảng khác
- venue_transfer_requests n-1 venue_clusters qua venue_cluster_id.
- venue_transfer_requests n-1 users qua from_owner_id.
- venue_transfer_requests n-1 users qua to_owner_id.
- venue_transfer_requests n-1 users qua requested_by.
- venue_transfer_requests n-1 users qua reviewed_by.

### 5. Ví dụ bản ghi
```json
{
    "id": "10000000-0000-0000-0000-000000000001",
    "transfer_code": "CODE-001",
    "venue_cluster_id": "10000000-0000-0000-0000-000000000001",
    "from_owner_id": "10000000-0000-0000-0000-000000000001",
    "to_owner_id": "10000000-0000-0000-0000-000000000001",
    "reason": "Nội dung mẫu",
    "status": "pending",
    "requested_by": "10000000-0000-0000-0000-000000000001"
}
...
## Tên bảng: administrative_units

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `administrative_units` theo schema migration hiện tại.

### 2. Danh sách trường

| STT | Tên trường | Kiểu dữ liệu | Null | Default | Khóa/Ràng buộc | Mô tả tác dụng trường | Ví dụ |
|---|---|---|---|---|---|---|---|
| 1 | id | unsignedInteger | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 1 |
| 2 | code | string(20) | Không | - | UNIQUE | Ma don vi hanh chinh. | CODE-001 |
| 3 | name | string(255) | Không | - | - | Ten don vi hanh chinh. | Ví dụ SportGo |
| 4 | name_en | string(255) | Có | - | - | Ten tieng Anh. | Ví dụ SportGo |
| 5 | full_name | string(500) | Có | - | - | Ten day du kem loai don vi. | Ví dụ SportGo |
| 6 | type | enum(province, district, ward) | Không | - | - | Loai don vi hanh chinh. | province |
| 7 | parent_code | string(20) | Có | - | - | Ma don vi cha. | CODE-001 |
| 8 | is_active | boolean | Không | true | - | Cờ xác định bản ghi đang hoạt động hay bị tắt. | true |
| 9 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 10 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- UNIQUE: administrative_units_code_unique (code)
- INDEX: administrative_units_type_parent_index (type, parent_code)
- INDEX: administrative_units_type_active_index (type, is_active)
...
```json
{
    ...
    "code": "CODE-001",
    "name": "Ví dụ SportGo",
    "name_en": "Ví dụ SportGo",
    "full_name": "Ví dụ SportGo",
    "type": "province",
    "parent_code": "CODE-001",
    "is_active": true
}
```
...
## Tên bảng: marketplace_listings

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `marketplace_listings` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | seller_id | char(36) | Không | - | FK | User dang ban/pass do. | 10000000-0000-0000-0000-000000000001 |
| 3 | title | string(255) | Không | - | - | Tieu de tin dang. | Ví dụ SportGo |
| 4 | description | text | Có | - | - | Mo ta chi tiet. | Nội dung mẫu |
| 5 | price | decimal(12,2) | Không | - | - | Gia de nghi. | 100000 |
| 6 | is_negotiable | boolean | Không | false | - | Co the thuong luong. | true |
| 7 | condition | enum(new, like_new, good, fair) | Không | good | - | Tinh trang san pham. | example |
| 8 | category | string(100) | Không | - | - | Danh muc san pham. | default |
| 9 | court_type_id | unsignedBigInteger | Có | - | FK | Mon the thao lien quan. | 1 |
| 10 | preferred_venue_cluster_id | char(36) | Có | - | FK, INDEX | Dia diem giao hang uu tien. | 10000000-0000-0000-0000-000000000001 |
| 11 | pickup_address | text | Có | - | - | Dia chi lay hang neu khong giao tai san. | example |
| 12 | status | enum(draft, active, sold, expired, hidden, rejected) | Không | draft | - | Trang thai tin dang. | draft |
| 13 | reviewed_by | char(36) | Có | - | FK | Admin kiem duyet. | 10000000-0000-0000-0000-000000000001 |
| 14 | reviewed_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `reviewed_at`. | 2026-06-22 09:00:00 |
| 15 | status_reason | text | Có | - | - | Lý do/ghi chú giải thích cho trạng thái hoặc quyết định xử lý. | active |
| 16 | view_count | unsignedBigInteger | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 17 | expires_at | timestamp | Có | - | INDEX | Thời điểm hết hạn hiệu lực. | 2026-06-22 09:00:00 |
| 18 | sold_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `sold_at`. | 2026-06-22 09:00:00 |
| 19 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 20 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- FK: court_type_id -> court_types.id (on delete: set null)
- FK: preferred_venue_cluster_id -> venue_clusters.id (on delete: set null)
- FK: reviewed_by -> users.id (on delete: set null)
- INDEX: marketplace_listings_seller_status_index (seller_id, status)
- INDEX: marketplace_listings_status_created_index (status, created_at)
- INDEX: marketplace_listings_category_status_index (category, status)
- INDEX: marketplace_listings_court_type_status_index (court_type_id, status)
- INDEX: marketplace_listings_preferred_cluster_index (preferred_venue_cluster_id)
- INDEX: marketplace_listings_expires_at_index (expires_at)

### 4. Quan hệ với bảng khác
- marketplace_listings n-1 users qua seller_id.
- marketplace_listings n-1 court_types qua court_type_id.
- marketplace_listings n-1 venue_clusters qua preferred_venue_cluster_id.
- marketplace_listings n-1 users qua reviewed_by.

### 5. Ví dụ bản ghi
```json
{
    "id": "10000000-0000-0000-0000-000000000001",
    "seller_id": "10000000-0000-0000-0000-000000000001",
    "title": "Ví dụ SportGo",
    "description": "Nội dung mẫu",
    "price": 100000,
    "is_negotiable": true,
...
## Tên bảng: court_membership_tiers

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `court_membership_tiers` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | venue_cluster_id | char(36) | Không | - | FK | Cum san cau hinh hang thanh vien. | 10000000-0000-0000-0000-000000000001 |
| 3 | tier | enum(standard, silver, gold, diamond) | Không | - | - | Hang thanh vien. | example |
| 4 | discount_percent | decimal(5,2) | Không | 0 | - | Phan tram giam gia ap dung. | 1 |
| 5 | min_bookings | unsignedInteger | Không | 0 | - | So booking toi thieu de dat hang. | 1 |
| 6 | min_spent_amount | decimal(12,2) | Không | 0 | - | Tong tien toi thieu de dat hang. | 100000 |
| 7 | maintain_min_bookings | unsignedInteger | Có | - | - | Booking toi thieu trong ky de duy tri hang. | 1 |
| 8 | maintain_min_spent | decimal(12,2) | Có | - | - | Tien toi thieu trong ky de duy tri hang. | 1 |
| 9 | maintain_period_months | unsignedTinyInteger | Có | - | - | Do dai ky duy tri tinh theo thang. | 1 |
| 10 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 11 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- PK: id
- FK: venue_cluster_id -> venue_clusters.id (on delete: cascade)
- UNIQUE: court_membership_tiers_cluster_tier_unique (venue_cluster_id, tier)

### 4. Quan hệ với bảng khác
- court_membership_tiers n-1 venue_clusters qua venue_cluster_id.
...
```json
{
    ...
    "maintain_min_bookings": 1,
    "maintain_min_spent": 1
}
```

---

## Tên bảng: court_type_requests

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `court_type_requests` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | unsignedBigInteger | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 1 |
| 2 | name | string(100) | Không | - | - | Tên/tiêu đề hiển thị hoặc dùng trong quản trị. | Ví dụ SportGo |
| 3 | description | text | Có | - | - | Mô tả hoặc ghi chú bổ sung cho bản ghi. | Nội dung mẫu |
| 4 | player_count | unsignedInteger | Không | 2 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 5 | default_layout_w | double | Có | - | - | Default width of court on layout canvas (cm) | 1 |
| 6 | default_layout_h | double | Có | - | - | Default height of court on layout canvas (cm) | 1 |
| 7 | parent_id | unsignedBigInteger | Có | - | FK | Khóa ngoại tham chiếu bảng `court_types`, dùng để liên kết bản ghi với dữ liệu liên quan. | 1 |
| 8 | requested_by | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 9 | status | enum(pending, approved, rejected) | Không | pending | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | pending |
...
### 4. Quan hệ với bảng khác
- court_type_requests n-1 court_types qua parent_id.
- court_type_requests n-1 users qua requested_by.
...
## Tên bảng: user_court_memberships

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `user_court_memberships` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | user_id | char(36) | Không | - | FK | User tich luy hang tai cum san. | 10000000-0000-0000-0000-000000000001 |
| 3 | venue_cluster_id | char(36) | Không | - | FK | Cum san ap dung hang thanh vien. | 10000000-0000-0000-0000-000000000001 |
| 4 | tier | enum(standard, silver, gold, diamond) | Không | standard | - | Hang hien tai. | example |
| 5 | total_bookings | unsignedInteger | Không | 0 | - | Tong booking da hoan thanh. | 1 |
| 6 | total_spent | decimal(14,2) | Không | 0 | - | Tong tien da chi. | 1 |
| 7 | period_bookings | unsignedInteger | Không | 0 | - | Booking trong ky duy tri hien tai. | 1 |
| 8 | period_spent | decimal(14,2) | Không | 0 | - | Tien chi trong ky duy tri hien tai. | 1 |
| 9 | period_start | date | Có | - | - | Ngay bat dau ky duy tri hien tai. | 2026-06-22 |
| 10 | last_upgraded_at | timestamp | Có | - | - | Thoi diem len hang gan nhat. | 2026-06-22 09:00:00 |
| 11 | last_downgraded_at | timestamp | Có | - | - | Thoi diem ha hang gan nhat. | 2026-06-22 09:00:00 |
| 12 | downgrade_notified_at | timestamp | Có | - | - | Thoi diem da thong bao ha hang. | 2026-06-22 09:00:00 |
| 13 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 14 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- FK: user_id -> users.id (on delete: cascade)
- FK: venue_cluster_id -> venue_clusters.id (on delete: cascade)
- UNIQUE: user_court_memberships_user_cluster_unique (user_id, venue_cluster_id)
- INDEX: user_court_memberships_cluster_tier_index (venue_cluster_id, tier)
- INDEX: user_court_memberships_user_tier_index (user_id, tier)

### 4. Quan hệ với bảng khác
- user_court_memberships n-1 users qua user_id.
- user_court_memberships n-1 venue_clusters qua venue_cluster_id.
...
## Tên bảng: membership_packages

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `membership_packages` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | name | string(100) | Không | - | - | Ten goi thanh vien he thong. | Ví dụ SportGo |
| 3 | type | enum(free, saving, pro) | Không | free | - | Loai goi. | free |
| 4 | monthly_price | decimal(12,2) | Có | - | - | Gia theo thang, null neu admin chua cau hinh. | 100000 |
| 5 | quarterly_price | decimal(12,2) | Có | - | - | Gia 3 thang. | 100000 |
| 6 | yearly_price | decimal(12,2) | Có | - | - | Gia 12 thang. | 100000 |
| 7 | voucher_count_per_month | unsignedTinyInteger | Không | 0 | - | So voucher VIP phat moi thang. | 1 |
| 8 | voucher_discount_percent | decimal(5,2) | Không | 0 | - | Phan tram giam cua voucher VIP. | 1 |
| 9 | voucher_min_order_amount | decimal(12,2) | Không | 0 | - | Gia tri don toi thieu de dung voucher VIP. | 100000 |
| 10 | voucher_max_discount_amount | decimal(12,2) | Có | - | - | Tran giam gia toi da moi voucher VIP. | 100000 |
| 11 | cashback_percent | decimal(5,2) | Không | 0 | - | Phan tram cashback vao vi sau booking. | 1 |
| 12 | match_post_limit_per_month | integer | Không | 5 | - | So bai tuyen giao luu moi thang, -1 la khong gioi han. | 1 |
| 13 | priority_complaint | boolean | Không | false | - | Uu tien khieu nai. | true |
| 14 | badge_name | string(100) | Có | - | - | Ten huy hieu hien thi tren profile. | Ví dụ SportGo |
| 15 | is_active | boolean | Không | true | - | Cờ xác định bản ghi đang hoạt động hay bị tắt. | true |
| 16 | sort_order | unsignedTinyInteger | Không | 0 | INDEX | Thu tu hien thi. | 1 |
| 17 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 18 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- INDEX: membership_packages_type_active_index (type, is_active)
- INDEX: membership_packages_sort_order_index (sort_order)
...
```json
{
    ...
    "quarterly_price": 100000,
    "yearly_price": 100000,
    "voucher_count_per_month": 1,
...
## Tên bảng: user_subscriptions

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `user_subscriptions` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | user_id | char(36) | Không | - | FK | User mua goi VIP. | 10000000-0000-0000-0000-000000000001 |
| 3 | package_id | char(36) | Không | - | FK | Goi thanh vien da mua. | 10000000-0000-0000-0000-000000000001 |
| 4 | billing_cycle | enum(monthly, quarterly, yearly) | Không | monthly | - | Chu ky da mua. | example |
| 5 | started_at | timestamp | Không | - | - | Thoi diem kich hoat. | 2026-06-22 09:00:00 |
| 6 | expires_at | timestamp | Không | - | - | Thoi diem het han. | 2026-06-22 09:00:00 |
| 7 | status | enum(pending_payment, active, expired, cancelled) | Không | pending_payment | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | pending_payment |
| 8 | paid_amount | decimal(12,2) | Không | 0 | - | Tien thuc te da tra. | 100000 |
| 9 | payment_ref | string(100) | Có | - | - | Ma thanh toan doi soat. | example |
| 10 | month_post_count | unsignedInteger | Không | 0 | - | So bai tuyen giao luu da dang trong thang. | 1 |
| 11 | month_post_reset_at | timestamp | Có | - | - | Thoi diem reset count thang moi. | 2026-06-22 09:00:00 |
| 12 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 13 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
- PK: id
- FK: user_id -> users.id (on delete: cascade)
- FK: package_id -> membership_packages.id (on delete: restrict)
- INDEX: user_subscriptions_user_status_index (user_id, status)
- INDEX: user_subscriptions_user_expires_index (user_id, expires_at)
- INDEX: user_subscriptions_status_expires_index (status, expires_at)

### 4. Quan hệ với bảng khác
- user_subscriptions n-1 users qua user_id.
- user_subscriptions n-1 membership_packages qua package_id.
...
## Tên bảng: system_wallet_balances

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `system_wallet_balances` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | system_bank_account_id | char(36) | Không | - | FK, UNIQUE | Tai khoan ngan hang he thong. | 10000000-0000-0000-0000-000000000001 |
| 3 | current_balance | decimal(18,2) | Không | 0 | - | So du hien tai doc tu tai khoan that. | 100000 |
| 4 | bank_balance | decimal(18,2) | Không | 0 | - | So du ATM doc tu SePay, chi dung de doi soat. | 100000 |
| 5 | refund_reserved_balance | decimal(18,2) | Không | 0 | - | Tien da cam ket hoan vao vi khach nhung chua rut khoi ATM. | 100000 |
| 6 | voucher_reserved_balance | decimal(18,2) | Không | 0 | - | Chi phi voucher he thong da cam ket cho chu san. | 100000 |
| 7 | last_synced_at | timestamp | Có | - | - | Thoi diem dong bo so du gan nhat. | 2026-06-22 09:00:00 |
| 8 | bank_synced_at | timestamp | Có | - | - | Thoi diem dong bo so du ATM gan nhat. | 2026-06-22 09:00:00 |
| 9 | alert_threshold | decimal(18,2) | Có | - | - | Nguong canh bao so du thap. | 1 |
| 10 | promotion_monthly_budget | decimal(15,2) | Có | - | - | Ngân sách khuyến mãi theo kỳ, chỉ dùng để cảnh báo. | 1 |
| 11 | budget_period_type | string(10) | Không | month | - | Kỳ ngân sách khuyến mãi: week, month hoặc year. | default |
| 12 | is_alert_enabled | boolean | Không | true | - | Bat tat canh bao. | true |
| 13 | last_alerted_at | timestamp | Có | - | - | Thoi diem gui canh bao gan nhat. | 2026-06-22 09:00:00 |
| 14 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 15 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: system_bank_account_id -> system_bank_accounts.id (on delete: cascade)
- UNIQUE: system_wallet_balances_system_bank_account_id_unique (system_bank_account_id)

### 4. Quan hệ với bảng khác
- system_wallet_balances n-1 system_bank_accounts qua system_bank_account_id.
...
```json
{
    ...
    "bank_balance": 100000,
    "refund_reserved_balance": 100000,
    "voucher_reserved_balance": 100000,
    "last_synced_at": "2026-06-22 09:00:00",
    "bank_synced_at": "2026-06-22 09:00:00"
}
```

---

## Tên bảng: system_wallet_ledgers

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `system_wallet_ledgers` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | system_bank_account_id | char(36) | Không | - | FK | Tai khoan ngan hang he thong. | 10000000-0000-0000-0000-000000000001 |
| 3 | transaction_ref | string(100) | Có | - | - | Ma giao dich tu ngan hang. | example |
| 4 | direction | enum(in, out) | Không | - | - | Chieu tien vao hoac ra. | example |
| 5 | entry_kind | string(40) | Không | actual | - | actual, refund_reserve, voucher_reserve, reserve_release. | example |
| 6 | amount | decimal(18,2) | Không | - | - | So tien giao dich. | 100000 |
| 7 | balance_before | decimal(18,2) | Không | - | - | So du truoc giao dich. | 100000 |
| 8 | balance_after | decimal(18,2) | Không | - | - | So du sau giao dich. | 100000 |
| 9 | refund_reserved_before | decimal(18,2) | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `system_wallet_ledgers`. | 1 |
| 10 | refund_reserved_after | decimal(18,2) | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `system_wallet_ledgers`. | 1 |
| 11 | voucher_reserved_before | decimal(18,2) | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `system_wallet_ledgers`. | 1 |
| 12 | voucher_reserved_after | decimal(18,2) | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `system_wallet_ledgers`. | 1 |
| 13 | transaction_type | enum(booking_payment, withdrawal_to_owner, refund_to_customer, platform_fee_received, adjustment, other) | Không | other | - | Phan loai nghiep vu. | booking_payment |
| 14 | reference_type | string(100) | Có | - | - | Loai doi tuong nghiep vu lien quan. | default |
| 15 | reference_id | string(100) | Có | - | - | ID doi tuong lien quan. | 1 |
| 16 | description | text | Có | - | - | Mo ta giao dich. | Nội dung mẫu |
| 17 | metadata | json | Có | - | - | JSON metadata bổ sung cho nghiệp vụ. | {"key":"value"} |
| 18 | transacted_at | timestamp | Không | - | - | Thoi diem giao dich thuc te. | 2026-06-22 09:00:00 |
| 19 | synced_at | timestamp | Không | - | - | Thoi diem he thong ghi nhan. | 2026-06-22 09:00:00 |
| 20 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: system_bank_account_id -> system_bank_accounts.id (on delete: restrict)
- INDEX: system_wallet_ledgers_account_time_index (system_bank_account_id, transacted_at)
- INDEX: system_wallet_ledgers_type_time_index (transaction_type, transacted_at)
- INDEX: system_wallet_ledgers_reference_index (reference_type, reference_id)
- INDEX: system_wallet_ledgers_direction_time_index (direction, transacted_at)
- UNIQUE: system_wallet_ledgers_account_ref_unique (system_bank_account_id, transaction_ref)
- INDEX: system_wallet_ledgers_kind_time_index (entry_kind, transacted_at)

### 4. Quan hệ với bảng khác
- system_wallet_ledgers n-1 system_bank_accounts qua system_bank_account_id.
...
## Tên bảng: venue_post_comments

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `venue_post_comments` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | venue_post_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `venue_posts`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
...
| 4 | parent_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `venue_post_comments`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 5 | content | text | Không | - | - | Nội dung chính của bản ghi. | Nội dung mẫu |
| 6 | status | enum(published, hidden, deleted) | Không | published | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | published |
| 7 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 8 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |
...
### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: venue_post_id -> venue_posts.id (on delete: cascade)
- FK: user_id -> users.id (on delete: cascade)
- FK: parent_id -> venue_post_comments.id (on delete: set null)
- INDEX: venue_post_comments_query_index (venue_post_id, status, created_at)

### 4. Quan hệ với bảng khác
- venue_post_comments n-1 venue_posts qua venue_post_id.
- venue_post_comments n-1 users qua user_id.
- venue_post_comments n-1 venue_post_comments qua parent_id.

### 5. Ví dụ bản ghi
```json
{
    "id": "10000000-0000-0000-0000-000000000001",
    "venue_post_id": "10000000-0000-0000-0000-000000000001",
    "user_id": "10000000-0000-0000-0000-000000000001",
    "parent_id": "10000000-0000-0000-0000-000000000001",
    "content": "Nội dung mẫu",
    "status": "published",
...
## Tên bảng: user_court_membership_histories

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `user_court_membership_histories` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | membership_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `user_court_memberships`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 3 | user_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 4 | venue_cluster_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `venue_clusters`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 5 | from_tier | enum(standard, silver, gold, diamond) | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `user_court_membership_histories`. | example |
| 6 | to_tier | enum(standard, silver, gold, diamond) | Không | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `user_court_membership_histories`. | example |
| 7 | change_type | string(30) | Không | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 8 | reason | string(255) | Có | - | - | Lý do/ghi chú giải thích cho trạng thái hoặc quyết định xử lý. | Nội dung mẫu |
| 9 | total_bookings | unsignedInteger | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 10 | total_spent | decimal(14,2) | Không | 0 | - | Số lượng/tổng số dùng cho thống kê hoặc giới hạn nghiệp vụ. | 1 |
| 11 | period_bookings | unsignedInteger | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `user_court_membership_histories`. | 1 |
| 12 | period_spent | decimal(14,2) | Không | 0 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `user_court_membership_histories`. | 1 |
| 13 | changed_at | timestamp | Không | - | - | Thời điểm xảy ra sự kiện `changed_at`. | 2026-06-22 09:00:00 |
| 14 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 15 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: membership_id -> user_court_memberships.id (on delete: set null)
- FK: user_id -> users.id (on delete: cascade)
- FK: venue_cluster_id -> venue_clusters.id (on delete: cascade)
- INDEX: user_court_membership_histories_user_cluster_changed_index (user_id, venue_cluster_id, changed_at)

### 4. Quan hệ với bảng khác
- user_court_membership_histories n-1 user_court_memberships qua membership_id.
- user_court_membership_histories n-1 users qua user_id.
- user_court_membership_histories n-1 venue_clusters qua venue_cluster_id.
...
## Tên bảng: document_signing_requests

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `document_signing_requests` theo schema migration hiện tại.
...
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | generated_document_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `generated_documents`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 3 | verification_code_id | unsignedBigInteger | Có | - | FK | Khóa ngoại tham chiếu bảng `verification_codes`, dùng để liên kết bản ghi với dữ liệu liên quan. | 1 |
| 4 | user_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 5 | signer_side | string(50) | Không | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | example |
| 6 | action | string(100) | Không | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | example |
| 7 | document_type | string(100) | Không | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 8 | document_code | string(50) | Không | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | CODE-001 |
| 9 | document_version | unsignedInteger | Không | 1 | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | 1 |
| 10 | file_hash | string(128) | Không | - | - | Đường dẫn hoặc URL tới tài nguyên liên quan. | example |
| 11 | file_hash_after | string(128) | Có | - | - | Đường dẫn hoặc URL tới tài nguyên liên quan. | example |
| 12 | nonce | string(100) | Không | - | UNIQUE | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | example |
| 13 | otp_type | string(150) | Không | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 14 | otp_channel | string(20) | Không | email | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | example |
| 15 | otp_identifier | string(255) | Không | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | example |
| 16 | otp_sent_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `otp_sent_at`. | 2026-06-22 09:00:00 |
| 17 | otp_verified_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `otp_verified_at`. | 2026-06-22 09:00:00 |
| 18 | expires_at | timestamp | Không | - | - | Thời điểm hết hạn hiệu lực. | 2026-06-22 09:00:00 |
| 19 | status | string(50) | Không | otp_sent | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | active |
| 20 | checkbox_text | text | Có | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `document_signing_requests`. | example |
| 21 | signature_image | longText | Có | - | - | Dữ liệu có cấu trúc hoặc nội dung dài phục vụ xử lý nghiệp vụ. | example |
| 22 | signed_signature_id | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `generated_document_signatures`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 23 | ip_address | string(45) | Có | - | - | Địa chỉ IP tại thời điểm thực hiện thao tác. | example |
| 24 | user_agent | string(500) | Có | - | - | Thông tin trình duyệt/thiết bị tại thời điểm thao tác. | example |
| 25 | metadata | json | Có | - | - | JSON metadata bổ sung cho nghiệp vụ. | {"key":"value"} |
...
### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: generated_document_id -> generated_documents.id (on delete: cascade)
- FK: verification_code_id -> verification_codes.id (on delete: set null)
- FK: user_id -> users.id (on delete: cascade)
- FK: signed_signature_id -> generated_document_signatures.id (on delete: set null)
- UNIQUE: document_signing_requests_nonce_unique (nonce)
- INDEX: doc_sign_req_doc_side_status_index (generated_document_id, signer_side, status)
- INDEX: doc_sign_req_user_status_index (user_id, status)
- INDEX: doc_sign_req_type_status_index (document_type, status)

### 4. Quan hệ với bảng khác
- document_signing_requests n-1 generated_documents qua generated_document_id.
- document_signing_requests n-1 verification_codes qua verification_code_id.
- document_signing_requests n-1 users qua user_id.
- document_signing_requests n-1 generated_document_signatures qua signed_signature_id.
...
## Tên bảng: platform_fee_email_logs

### 1. Mục đích bảng
Lưu dữ liệu nghiệp vụ cho bảng `platform_fee_email_logs` theo schema migration hiện tại.
...
|---|---|---|---|---|---|---|---|
| 1 | id | char(36) | Không | - | PK | Khóa chính định danh duy nhất của bản ghi. | 10000000-0000-0000-0000-000000000001 |
| 2 | ledger_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `venue_platform_fee_ledgers`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 3 | venue_cluster_id | char(36) | Không | - | FK | Khóa ngoại tham chiếu bảng `venue_clusters`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 4 | type | string(50) | Không | - | - | Loại hoặc nhóm phân loại của bản ghi. | default |
| 5 | email | string(255) | Có | - | - | Địa chỉ email dùng để đăng nhập, liên hệ hoặc nhận thông báo. | user@sportgo.vn |
| 6 | subject | string(255) | Không | - | - | Trường dữ liệu phục vụ nghiệp vụ của bảng `platform_fee_email_logs`. | example |
| 7 | content | text | Có | - | - | Nội dung chính của bản ghi. | Nội dung mẫu |
| 8 | status | enum(queued, sent, failed, skipped) | Không | queued | - | Trạng thái xử lý hoặc vòng đời của bản ghi. | queued |
| 9 | queued_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `queued_at`. | 2026-06-22 09:00:00 |
| 10 | sent_at | timestamp | Có | - | - | Thời điểm xảy ra sự kiện `sent_at`. | 2026-06-22 09:00:00 |
| 11 | error_reason | text | Có | - | - | Lý do/ghi chú giải thích cho trạng thái hoặc quyết định xử lý. | Nội dung mẫu |
| 12 | triggered_by | char(36) | Có | - | FK | Khóa ngoại tham chiếu bảng `users`, dùng để liên kết bản ghi với dữ liệu liên quan. | 10000000-0000-0000-0000-000000000001 |
| 13 | metadata | json | Có | - | - | JSON metadata bổ sung cho nghiệp vụ. | {"key":"value"} |
| 14 | created_at | timestamp | Có | - | - | Thời điểm tạo bản ghi. | 2026-06-22 09:00:00 |
| 15 | updated_at | timestamp | Có | - | - | Thời điểm cập nhật bản ghi gần nhất. | 2026-06-22 09:00:00 |

### 3. Khóa chính, khóa ngoại, index
- PK: id
- FK: ledger_id -> venue_platform_fee_ledgers.id (on delete: cascade)
- FK: venue_cluster_id -> venue_clusters.id (on delete: cascade)
- FK: triggered_by -> users.id (on delete: set null)
- INDEX: pf_email_logs_ledger_type_status_index (ledger_id, type, status)
- INDEX: pf_email_logs_status_created_at_index (status, created_at)

### 4. Quan hệ với bảng khác
- platform_fee_email_logs n-1 venue_platform_fee_ledgers qua ledger_id.
- platform_fee_email_logs n-1 venue_clusters qua venue_cluster_id.
- platform_fee_email_logs n-1 users qua triggered_by.
