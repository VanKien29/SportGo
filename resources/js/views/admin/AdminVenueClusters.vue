<template>
  <div class="avc-page">
    ...
    <div v-else-if="error" class="state-box card error-box">
      <p>{{ error }}</p>
      <button class="btn btn-outline" @click="loadClusters">Thử lại</button>
    </div>
        ...
        <table class="avc-table">
          ...
          <tbody>
            ...
            >
              ...
              <td>
                <div class="owner-name">{{ c.owner?.full_name || '—' }}</div>
                <div class="owner-email">{{ c.owner?.email || '' }}</div>
              </td>
              <td class="address-cell">{{ c.address }}</td>
              <td>
                <div class="court-types">
                  <span v-for="(ct, i) in (c.court_types || [])" :key="i" class="ct-chip">{{ ct }}</span>
                  <span v-if="!c.court_types || c.court_types.length === 0" class="text-muted">—</span>
                </div>
              </td>
              <td class="text-center">
                <span class="court-count">{{ c.court_count }}</span>
              </td>
              <td class="text-center">
                <span class="rating">
                  {{ Number(c.rating_avg || 0).toFixed(1) }}
                  <span class="rating-count">({{ c.rating_count }})</span>
                </span>
              </td>
              <td class="text-center">
              ...
              <td class="text-right" @click.stop>
                <button class="btn-action btn-view" @click="goDetail(c.id)">Chi tiết</button>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    statusLabel(status) {
      const map = { pending: 'Chờ duyệt', active: 'Hoạt động', locked: 'Đã khóa' };
      return map[status] || status;
    },
    goDetail(id) {
      this.$router.push({ name: 'admin-venue-cluster-detail', params: { id } });
    },
  },
};
</script>
...
/* Table */
.avc-table-wrap { padding: 0; overflow: hidden; }
...
.avc-row:hover { background: #f8fafc; }

.cluster-name { font-weight: 700; color: var(--sg-text); }
.cluster-slug { font-size: 12px; color: rgba(15, 23, 42, 0.4); margin-top: 2px; }
.owner-name { font-weight: 600; }
.owner-email { font-size: 12px; color: rgba(15, 23, 42, 0.5); }
.address-cell { max-width: 200px; color: rgba(15, 23, 42, 0.7); font-size: 13px; }

.court-types { display: flex; flex-wrap: wrap; gap: 4px; }
.ct-chip {
  padding: 3px 8px;
  border-radius: 6px;
  background: #f1f5f9;
...
.court-count { font-weight: 700; font-size: 15px; }
.rating { font-size: 13px; white-space: nowrap; }
.rating-count { color: rgba(15, 23, 42, 0.4); font-size: 12px; }
.text-muted { color: rgba(15, 23, 42, 0.4); }
