  ...
  <section class="system-wallet-page" :class="{ embedded }">
    <header v-if="!embedded" class="page-header">
      <div>
        ...
        <p>Theo dõi doanh thu, chi phí khuyến mãi, ngân sách cảnh báo và lịch sử đối soát ATM.</p>
      </div>
      <button class="sync-button" type="button" :disabled="loading || syncing" @click="syncWallet">
    ...
    <section v-if="!embedded" class="account-card">
      <div>
        ...
        <p>{{ account?.account_number || '-' }} · {{ account?.account_holder || account?.account_holder_name || '-' }}</p>
      </div>
      <div class="sync-meta">
    ...
    <section class="metric-grid">
      ...
      <article class="metric-card" :class="{ danger: wallet?.is_low_balance }">
        <span>Lãi ròng tham chiếu</span>
        <strong>{{ money(netRevenue) }}</strong>
      </article>
      <article class="metric-card warning" :class="{ danger: promotionBudget?.is_over_budget }">
    ...
    <section class="finance-grid">
      <article class="finance-card">
        ...
        <div class="finance-row">
          ...
          <strong>{{ money(revenueSummary?.platform_fees?.total_paid) }}</strong>
        </div>
        <div class="finance-row">
          <span>Booking online</span>
          <strong>{{ money(revenueSummary?.booking_payments?.total) }}</strong>
        </div>
        <div class="finance-row muted">
          <span>Phí nền tảng quá hạn</span>
          <strong>{{ money(revenueSummary?.platform_fees?.overdue_amount) }}</strong>
        </div>
      </article>
      ...
      <article class="finance-card">
        ...
        <div class="finance-row">
          ...
          <strong>{{ money(promotionExpenses?.voucher_total) }}</strong>
        </div>
        <div class="finance-row">
          <span>Hoàn vào ví</span>
          <strong>{{ money(promotionExpenses?.refund_total) }}</strong>
        </div>
        <div class="finance-row muted">
    ...
    <section class="settings-card">
      <div>
        ...
        <p>Ngân sách chỉ dùng để cảnh báo, không chặn khách dùng voucher hệ thống.</p>
      </div>
      <form class="settings-form" @submit.prevent="saveSettings">
        <label class="toggle-row">
          <input v-model="settings.is_alert_enabled" type="checkbox" />
          <span>Bật cảnh báo</span>
        </label>
        <label class="amount-input">
          <span>Ngân sách</span>
          <input v-model.number="settings.promotion_budget" type="number" min="0" step="1000" />
        </label>
        <label class="amount-input">
        ...
        <button class="secondary-button" type="submit" :disabled="saving">
    ...
    <section class="ledger-card">
      <div class="ledger-head">
        <div>
          ...
          <p>Nạp quỹ, trừ voucher hệ thống và giao dịch ATM dùng để đối soát.</p>
        </div>
        <form class="filters" @submit.prevent="loadWallet(1)">
          ...
          <select v-model="filters.entry_kind">
            ...
            <option value="voucher_subsidy">Trừ voucher hệ thống</option>
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            <tr v-if="loading">
              <td colspan="7" class="empty-row">Đang tải ví hệ thống...</td>
            </tr>
            <tr v-else-if="!ledgers.length">
              <td colspan="7" class="empty-row">Chưa có lịch sử ví hệ thống.</td>
            </tr>
            <template v-else>
              <tr v-for="ledger in ledgers" :key="ledger.id">
                <td>{{ formatDateTime(ledger.transacted_at) }}</td>
                <td>
                  <span class="badge" :class="ledger.direction === 'in' ? 'in' : 'out'">
                    {{ ledger.direction === 'in' ? 'Tiền vào' : 'Tiền ra' }}
                  </span>
                </td>
                <td>{{ kindLabel(ledger.entry_kind) }}</td>
                <td class="amount" :class="ledger.direction">{{ money(ledger.amount) }}</td>
                <td>
                  <strong>{{ money(ledger.balance_after) }}</strong>
                  <small>Trước: {{ money(ledger.balance_before) }}</small>
                </td>
                <td>
                  <strong>{{ ledger.transaction_ref || '-' }}</strong>
                  <small>{{ ledger.reference_type || '-' }} · {{ ledger.reference_id || '-' }}</small>
                </td>
      ...
      <div class="pagination" v-if="meta.last_page > 1">
        <button type="button" :disabled="meta.current_page <= 1" @click="loadWallet(meta.current_page - 1)">Trước</button>
        <span>Trang {{ meta.current_page }} / {{ meta.last_page }}</span>
        <button type="button" :disabled="meta.current_page >= meta.last_page" @click="loadWallet(meta.current_page + 1)">Sau</button>
      </div>
    </section>
  </section>
</template>
...
export default {
  ...
  computed: {
    netRevenue() {
      return Number(this.revenueSummary?.total_revenue || 0) - Number(this.promotionExpenses?.total || 0);
    },
    usageText() {
      if (this.promotionBudget?.usage_percent === null || this.promotionBudget?.usage_percent === undefined) {
        return 'Chưa đặt';
      }
  ...
  methods: {
    async loadWallet(page = 1) {
      ...
      try {
        ...
        this.settings = {
          is_alert_enabled: Boolean(response.wallet?.is_alert_enabled),
          promotion_budget: Number(response.wallet?.promotion_monthly_budget || 0),
          budget_period: response.wallet?.budget_period_type || 'month',
        };
      } catch (error) {
    ...
    async saveSettings() {
      ...
      this.success = '';
      try {
        const response = await adminSystemWalletService.updateSettings(this.settings);
        this.wallet = response.wallet;
        this.success = response.message || 'Đã lưu ngân sách khuyến mãi.';
        await this.loadWallet(this.meta.current_page);
      } catch (error) {
        this.error = error.message || 'Không lưu được ngân sách khuyến mãi.';
      } finally {
        this.saving = false;
      }
    },
