<template>
    <section class="admin-page">
        ...
        <template v-else-if="!policy">
            ...
            <button
                class="btn secondary"
                type="button"
                @click="$router.push({ name: 'admin-policies' })"
            >
        ...
        <template v-else>
            <header class="policy-detail-header">
                <div class="policy-detail-heading">
                    <div class="policy-title-row">
                        ...
                        >
                        <span class="policy-version-text"
                            >v{{ policy.version || 1 }}</span
                        >
                    </div>
            ...
            <nav class="tab-nav" aria-label="Các phần của chính sách">
                <button
                    v-for="tab in tabs"
                    :key="tab.key"
                    class="tab-btn"
                ...
                <div class="meta-grid">
                    <div class="meta-item span2">
                        ...
                        <span class="meta-value">{{
                            policy.business_summary_vi ||
                            policy.business_summary ||
                            "(Chưa có mô tả)"
                        }}</span>
                    </div>
                    <div class="meta-item">
                    ...
                    <div class="meta-item">
                        ...
                        <span class="meta-value">{{
                            formatDate(
                                policy.effective_from || policy.published_at,
                            )
                        }}</span>
                    </div>
                    <div class="meta-item">
                        ...
                        <span
                            class="meta-value"
                            :class="{ 'text-green': policy.require_reaccept }"
                            >{{
                    ...
                    <div class="meta-item">
                        ...
                        <span
                            class="meta-value"
                            :class="{ 'text-green': policy.is_overridable }"
                            >{{
                                policy.is_overridable
                                    ? "Có (trong khung hệ thống)"
                                    : "Không"
                            }}</span
                        >
                    </div>
            ...
            <div v-if="activeTab === 'content'" class="tab-body">
                <div class="section-row">
                    <div>
                        ...
                        <p>
                            Đây là nội dung người dùng và chủ sân sẽ đọc khi cần
                            đồng ý.
                        </p>
                    </div>
                    <button
                ...
                <div class="content-editor-wrap">
                    ...
                    <div
                        v-else
                        class="content-readonly ql-editor"
                        v-html="contentDraft"
                    ></div>
                </div>
            </div>
            ...
            <div v-if="activeTab === 'config'" class="tab-body">
                ...
                <div
                    v-if="policy.status === 'active'"
                    class="info-notice warning"
                >
                    <AppIcon name="lock" size="15" />
                    Chính sách đang áp dụng. Tạo phiên bản mới để chỉnh sửa cấu
                    hình.
                </div>

                <div
                    v-if="configurationType === 'text_only'"
                    class="info-notice"
                >
                    <AppIcon name="fileText" size="15" />
                    Chính sách này chỉ là nội dung hiển thị, không có xử lý tự
                    động.
                </div>

                <article
                    v-else-if="configurationType === 'cancel_refund_tiers'"
                    class="config-card"
                >
                    <div class="config-head">
                        <div>
                            <span class="config-kicker"
                                >Hủy & hoàn booking</span
                            >
                            <h3>Bảng mốc thời gian hủy và hoàn</h3>
                            <p>
                                {{
                                    cancelRefundConfiguration?.summary ||
                                    "Chưa cấu hình bảng mốc hủy và hoàn."
                                }}
                            </p>
                        </div>
                        <button
                    ...
                    <div class="config-table-wrap">
                        <table class="config-table">
                            ...
                            <tbody>
                                <tr
                                    v-for="tier in cancelRefundRows"
                                    :key="tier.key"
                                >
                                    <td>
                    ...
                    <div
                        class="policy-rule-note"
                        :class="{ disabled: !ownerFaultFullRefund.enabled }"
                    >
                        <div>
                            <span class="config-kicker"
                                >Trường hợp lỗi phía sân</span
                            >
                            <strong>{{
                                ownerFaultFullRefund.rule_name
                            }}</strong>
                ...
                <AdminModerationConfig
                    v-else-if="configurationType === 'moderation_thresholds'"
                    :policy-id="policy.id"
                ...
                <article v-else class="config-card">
                    <div class="config-head">
                        ...
                        <button
                            ...
                            type="button"
                            :disabled="
                                !canEditConfig || !genericConfigFields.length
                            "
                            @click="openGenericConfigModal"
                        >
                            <AppIcon name="pencil" size="15" />
                            Sửa cấu hình
                        </button>
                    ...
                    <div class="meta-grid">
                        <div
                            v-for="entry in genericConfigEntries"
                            :key="entry.key"
                            class="meta-item"
                        >
            ...
            <div v-if="activeTab === 'audit'" class="tab-body">
                <div class="section-row">
                    <div>
                        ...
                        <p>
                            Ghi nhận các thao tác chỉnh sửa, kích hoạt và tạo
                            phiên bản mới.
                        </p>
                    </div>
                </div>
                ...
                <div class="audit-list">
                    ...
                    >
                        ...
                        <div class="audit-content">
                            <strong>{{
                                log.human_message || "Đã cập nhật chính sách"
                            }}</strong>
                            <span class="audit-meta"
                                >{{ formatDateTime(log.created_at) }} ·
                                {{ log.actor_name || "Hệ thống" }}</span
                            >
                            <ul
                                v-if="log.changes_summary?.length"
                                class="change-list"
                            >
                                <li
                                    v-for="c in log.changes_summary"
                                    :key="c.field"
                                >
                                    {{ c.summary }}
                                </li>
        ...
        <div v-if="showWizard" class="modal-bg" @click.self="closeWizard">
            <div class="modal-box">
                ...
                <div class="modal-head">
                    ...
                    <button class="icon-btn" @click="closeWizard">
                ...
                <div v-if="wizardStep === 1" class="modal-body">
                    <p class="step-hint">
                        Chọn tình huống mà quy tắc sẽ được áp dụng trong hệ
                        thống.
                    </p>
                    ...
                    >
                        ...
                        <span
                            >Chưa có tình huống phù hợp cho nhóm
                            <strong>{{ policy?.policy_type_label }}</strong
                            >.</span
                        >
                    </div>
                    ...
                    <div v-else class="option-list">
                        <label
                            ...
                            :class="{
                                selected:
                                    ruleForm.action_code === action.action_code,
                            }"
                        >
                            <input
                                type="radio"
                            ...
                            <div class="option-card-body">
                                <strong>{{
                                    action.label ||
                                    action.action_label_vi ||
                                    getActionLabel(action.action_code)
                                }}</strong>
                                <span
                                    >Chọn tình huống nghiệp vụ này để tiếp tục
                                    cấu hình quy tắc.</span
                                >
                            </div>
                        </label>
                    </div>
                ...
                <div v-if="wizardStep === 2" class="modal-body">
                    ...
                    <div class="selected-context">
                        ...
                        <span
                            >Tình huống:
                            <strong>{{
                                getActionLabel(ruleForm.action_code)
                            }}</strong></span
                        >
                        <button class="link-btn" @click="wizardStep = 1">
                            Thay đổi
                        </button>
                    </div>
                    ...
                    <div v-else class="option-list compact">
                        <label
                            ...
                            class="option-card"
                            :class="{
                                selected: ruleForm.rule_type === tpl.rule_type,
                            }"
                        ...
                        >
                            ...
                            <div class="option-card-body">
                                <strong>{{
                                    tpl.label || getRuleTypeLabel(tpl.rule_type)
                                }}</strong>
                                <span>{{
                                    tpl.business_summary_vi ||
                                    tpl.description ||
                                    ""
                                }}</span>
                            </div>
                    ...
                    <template v-if="ruleForm.rule_type">
                        <div v-if="hasConfigFields" class="param-section">
                            ...
                            <div class="param-grid">
                                <label
                                    v-if="needsField('hours_before_start')"
                                    class="param-field"
                                >
                                    ...
                                    <div class="input-unit">
                                        <input
                                            v-model.number="
                                                formNumbers.hours_before_start
                                            "
                                            type="number"
                                            min="1"
                                        />
                                ...
                                <label
                                    v-if="needsField('refund_percent')"
                                    class="param-field"
                                >
                                ...
                                <label
                                    v-if="needsField('days_before_due')"
                                    class="param-field"
                                >
                                ...
                                <label
                                    v-if="needsField('unique_reporters')"
                                    class="param-field"
                                >
                                ...
                                <label
                                    v-if="needsField('transition_days')"
                                    class="param-field"
                                >
                                ...
                                <label
                                    v-if="needsField('owner_confirm_required')"
                                    class="param-field-check"
                                >
                                    <input
                                        v-model="
                                            formBooleans.owner_confirm_required
                                        "
                                        type="checkbox"
                                    />
                                    <span
                                        >Bắt buộc chủ sân xác nhận trước khi
                        ...
                        <div class="name-fields">
                            <label class="param-field">
                                <span
                                    ...
                                    <em>(tuỳ chỉnh)</em></span
                                >
                                <input
                                    v-model.trim="ruleForm.rule_name"
                                    type="text"
                                    :placeholder="
                                        getRuleTypeLabel(ruleForm.rule_type)
                                    "
                                />
                            </label>
                        </div>
                ...
                <div class="modal-foot">
                    <button
                        class="btn secondary"
                        @click="
                            wizardStep === 1 ? closeWizard() : (wizardStep = 1)
                        "
                    >
                    ...
                    <div class="modal-foot-right">
                        ...
                        <template v-else>
                            <button
                                ...
                                :disabled="savingRule || !ruleForm.rule_type"
                            >
    ...
    ════════════════════════════════════ -->
        <div
            v-if="cancelRefundModal"
            class="modal-bg"
            @click.self="closeConfigModals"
        >
            <form
                class="modal-box wide"
                @submit.prevent="saveCancelRefundConfig"
            >
                <div class="modal-head">
                    <div>
                ...
                <div class="modal-body">
                    ...
                    <label class="owner-fault-config">
                        <input
                            v-model="ownerFaultFullRefundDraft"
                            type="checkbox"
                        />
                        <span>
                            <strong
                                >Hoàn 100% khi lỗi phát sinh từ phía sân</strong
                            >
                            <small
                                >Áp dụng cho chủ sân hủy, khóa sân hoặc bảo trì
                                làm ảnh hưởng booking đã thanh toán; tiền hoàn
                                vào ví SportGo của khách.</small
                            >
                        </span>
                    </label>
                    <article
                        v-for="tier in cancelRefundDraft"
                        :key="tier.key"
                        class="config-edit-row"
                    >
                        <div class="config-edit-grid cancel-grid">
                            <label>
                                Từ giờ
                                <input
                                    v-model.number="tier.from_hours"
                                    type="number"
                                    min="0"
                                    step="0.5"
                                />
                            ...
                            <label class="check-row">
                                <input
                                    v-model="tier.require_owner_confirm"
                                    type="checkbox"
                                />
                                Chủ sân xác nhận
                            </label>
                            <label class="wide-field">
                                Nội dung cho khách
                                <input
                                    v-model.trim="tier.customer_message"
                                    maxlength="500"
                                    placeholder="Ví dụ: Yêu cầu hủy nằm ngoài khung được hoàn tiền."
                                />
                            </label>
                            <button
                                ...
                                type="button"
                                title="Xóa mốc"
                                :disabled="cancelRefundDraft.length <= 2"
                                @click="removeCancelRefundTier(tier.key)"
                            >
                    ...
                    <div
                        ...
                        :class="{ invalid: cancelRefundValidation }"
                    >
                        ...
                        <p>
                            {{ cancelRefundValidation || cancelRefundPreview }}
                        </p>
                    </div>
                </div>
                <div class="modal-foot">
                    ...
                    <button
                        class="btn primary"
                        type="submit"
                        :disabled="savingRule || !!cancelRefundValidation"
                    >
            ...
            <form class="modal-box wide" @submit.prevent="saveGenericConfig">
                ...
                <div class="modal-body">
                    <div class="config-edit-grid generic-grid">
                        <label
                            v-for="field in genericConfigFields"
                            :key="field.key"
                            :class="{
                                'wide-field':
                                    field.type === 'textarea' ||
                        ...
                        >
                            ...
                            <template v-if="field.type === 'boolean'">
                                <span class="switch-row">
                                    <input
                                        v-model="genericConfigDraft[field.key]"
                                        type="checkbox"
                                    />
                                    {{
                                        genericConfigDraft[field.key]
                                            ? "Có"
                                            : "Không"
                                    }}
                                </span>
                            ...
                            <textarea
                                ...
                                v-model.trim="genericConfigDraft[field.key]"
                                rows="3"
                            ></textarea>
                            <input
                                v-else-if="field.type === 'number'"
                                v-model.number="genericConfigDraft[field.key]"
                                type="number"
                                min="0"
                            />
                            <input
                    ...
                    <div
                        class="preview-banner"
                        :class="{ invalid: genericConfigValidation }"
                    >
                        <p class="preview-label">
                            {{
                        ...
                        <p>
                            {{
                                genericConfigValidation ||
                                "Cấu hình sẽ được lưu vào rule của chính sách hiện tại."
                            }}
                        </p>
                    </div>
                </div>
                <div class="modal-foot">
                    ...
                    <button
                        class="btn primary"
                        type="submit"
                        :disabled="savingRule || !!genericConfigValidation"
                    >
    ...
    ════════════════════════════════════ -->
        ...
        >
            <div class="modal-box confirm-box">
                ...
                <p>
                    Chính sách <strong>{{ policy?.title }}</strong> và tất cả
                    quy tắc liên quan sẽ bị xóa vĩnh viễn.
                </p>
...
export default {
    ...
    computed: {
        ...
        filteredActions() {
            return this.actionOptions.filter((a) =>
                (a.policy_types || []).includes(this.policyType),
            );
        },
        filteredTemplates() {
            return this.ruleTemplates.filter((t) => {
                ...
                const actionOk =
                    !this.ruleForm.action_code ||
                    (t.action_codes || []).includes(this.ruleForm.action_code);
                return typeOk && actionOk;
            });
        },
        selectedTemplate() {
            return this.ruleTemplates.find(
                (t) => t.rule_type === this.ruleForm.rule_type,
            );
        },
        hasConfigFields() {
        ...
        previewSummary() {
            ...
            const m = {
                ...
                contract_signing_required:
                    "Hợp đồng chỉ có hiệu lực khi đã có đủ chữ ký.",
                partner_termination_transition_30_days: `Thu quyền chủ sân sau ${n.transition_days} ngày chuyển tiếp.`,
                terms_acceptance_required:
                    "Bắt buộc chấp nhận điều khoản trước khi sử dụng dịch vụ.",
                venue_policy_override_limit:
                    "Chính sách riêng của sân không được vượt quá khung hệ thống đã đặt.",
                partner_application_approve_requires_contract:
                    "Duyệt hồ sơ đối tác xong phải tạo hợp đồng trước khi chính thức..",
            };
            return (
                m[t] ||
                this.selectedTemplate?.business_summary_vi ||
                "Chọn loại quy tắc để xem mô tả nghiệp vụ."
            );
        },
        canEditConfig() {
        ...
        configurationType() {
            return (
                this.detail?.configuration_type ||
                this.policy?.configuration_type ||
                "text_only"
            );
        },
        configurationSummary() {
            return (
                this.detail?.business_summary ||
                this.policy?.business_summary_vi ||
                this.policy?.business_summary ||
                "Cấu hình cách hệ thống tự xử lý chính sách."
            );
        },
        ...
        ownerFaultFullRefund() {
            return (
                this.cancelRefundConfiguration?.owner_fault_full_refund || {
                    enabled: true,
        ...
        moderationConfiguration() {
            return (
                this.detail?.moderation_thresholds ||
                this.detail?.report_configuration ||
                null
            );
        },
        genericConfiguration() {
            const byType = {
                ...
                permission_revoke:
                    this.policy?.configuration_data ||
                    this.detail?.permission_revoke_configuration?.config,
                partner_contract:
                    this.policy?.configuration_data ||
                    this.detail?.partner_contract_configuration?.config,
                account_policy:
                    this.policy?.configuration_data ||
                    this.detail?.account_policy_configuration?.config,
            };
            return byType[this.configurationType] || {};
        },
        configCount() {
            if (this.configurationType === "cancel_refund_tiers")
                return (
                    this.cancelRefundConfiguration?.tiers?.length ||
                    this.rules.length
                );
            if (this.configurationType === "moderation_thresholds")
                return this.reportThresholdRows.length || this.rules.length;
            if (this.configurationType === "text_only") return 0;
            return Object.keys(this.genericConfiguration || {}).length;
        },
        cancelRefundRows() {
            return (this.cancelRefundConfiguration?.tiers || []).map(
                (tier, index) => ({
                    key: tier.key || `tier_${index}`,
                    label: tier.label || this.dynamicTierLabel(tier),
                    condition:
                        tier.condition_label || this.dynamicTierCondition(tier),
        ...
        reportTargetOptions() {
            return (
                this.moderationConfiguration?.target_type_options || [
        ...
        reportThresholdRows() {
            if (this.moderationConfiguration?.thresholds?.length) {
                return this.moderationConfiguration.thresholds.map(
                    (threshold, index) =>
                        this.normalizeThresholdShape(threshold, index),
                );
            }
            ...
            return [
                this.normalizeThresholdShape(
                    {
                        key: "legacy",
                        object_type:
                            this.moderationConfiguration?.target_type || "post",
                        object_type_label:
                            this.moderationConfiguration?.target_type_label ||
                            "Nội dung",
                        ...
                        action_label:
                            (
                                this.moderationConfiguration?.action_labels ||
                                []
                            ).join(", ") || "Thông báo admin",
                        notify_admin: (config.actions || []).includes(
                            "notify_admin",
                        ),
        ...
        genericConfigSummary() {
            ...
            return (
                ...
                this.detail?.permission_revoke_configuration?.summary ||
                this.detail?.partner_contract_configuration?.summary ||
                this.detail?.account_policy_configuration?.summary ||
                "Cấu hình này được lưu dưới dạng rule của chính sách."
            );
        },
        genericConfigFields() {
            const fields = {
                ...
                partner_contract: [
                    [
                        "warn_before_days",
                        "Thông báo trước khi hết hạn (ngày)",
                        "number",
                    ],
                    [
                        "lock_after_days",
                    ...
                    [
                        "revoke_after_days",
                        "Chuyển Admin xử lý thu hồi quyền (ngày)",
                        "number",
                    ],
                ],
                account_policy: [
                    [
                        "warnings_to_restrict",
                        "Số cảnh báo trước khi hạn chế",
                        "number",
                    ],
                    [
                        "violations_to_lock",
                        "Số vi phạm trước khi khóa",
                        "number",
                    ],
            ...
            return (fields[this.configurationType] || []).map(
        ...
        cancelRefundPreview() {
            if (!this.cancelRefundDraft.length)
                return "Thêm ít nhất 2 mốc thời gian để lưu chính sách.";
            return (
                this.sortedCancelRefundDraft
                    .map(
                        (tier) =>
                            `${this.dynamicTierLabel(tier)}: ${this.cancelRefundResult(tier).toLowerCase()}`,
                    )
                    .join(". ") + "."
            );
        },
        sortedCancelRefundDraft() {
            return [...this.cancelRefundDraft].sort(
                (a, b) => Number(a.from_hours || 0) - Number(b.from_hours || 0),
            );
        },
        cancelRefundValidation() {
            if (this.cancelRefundDraft.length < 2)
                return "Bảng mốc cần ít nhất 2 dòng.";
            const sorted = this.sortedCancelRefundDraft.map((tier) => ({
                from: Number(tier.from_hours),
                to:
                    tier.to_hours === "" ||
                    tier.to_hours === null ||
                    tier.to_hours === undefined
                        ? null
                        : Number(tier.to_hours),
                allowCancel: !!tier.allow_cancel,
                refundPercent: Number(tier.refund_percent),
            }));
            if (sorted.some((tier) => Number.isNaN(tier.from) || tier.from < 0))
                return "Giờ bắt đầu phải lớn hơn hoặc bằng 0.";
            if (
                sorted.some(
                    (tier) =>
                        tier.to !== null &&
                        (Number.isNaN(tier.to) || tier.to <= tier.from),
                )
            )
            ...
            if (
                sorted.some(
                    (tier) =>
                        Number.isNaN(tier.refundPercent) ||
            ...
            if (
                sorted.some(
                    (tier) => !tier.allowCancel && tier.refundPercent !== 0,
                )
            )
                return "Mốc không cho hủy phải có tỷ lệ hoàn bằng 0%.";
        ...
        reportPreview() {
            if (!this.reportThresholdDraft.length)
                return "Thêm ít nhất một ngưỡng báo cáo.";
            return (
                this.reportThresholdDraft
                    .map((threshold) => {
                        const action =
                            this.reportActionOptionsFor(
                                threshold.object_type,
                            ).find((item) => item.value === threshold.action)
        ...
        reportValidation() {
            if (!this.reportThresholdDraft.length)
                return "Vui lòng thêm ít nhất một ngưỡng báo cáo.";
            for (const [
                index,
                threshold,
            ] of this.reportThresholdDraft.entries()) {
                if (
                    Number(threshold.min_reports) < 1 ||
                    Number(threshold.min_distinct_reporters) < 1 ||
                    Number(threshold.within_days) < 1
                ) {
                    return `Dòng ${index + 1}: các ngưỡng phải lớn hơn 0.`;
                }
                if (
                    Number(threshold.min_distinct_reporters) >
                    Number(threshold.min_reports)
                ) {
                    return `Dòng ${index + 1}: số người báo cáo không được lớn hơn số báo cáo.`;
                }
                if (
                    !this.reportActionOptionsFor(threshold.object_type).some(
                        (option) => option.value === threshold.action,
                    )
                ) {
        ...
        genericConfigValidation() {
            ...
            if (this.configurationType === "platform_fee") {
                ...
                for (const key of ["remind_before_days", ...order]) {
                    if (Number(this.genericConfigDraft[key]) < 0)
                        return "Các mốc ngày phải lớn hơn hoặc bằng 0.";
                }
                for (let index = 0; index < order.length - 1; index += 1) {
                    if (
                        Number(this.genericConfigDraft[order[index]]) >
                        Number(this.genericConfigDraft[order[index + 1]])
                    ) {
            ...
            if (
                this.configurationType === "partner_contract" &&
                Number(this.genericConfigDraft.lock_after_days) >
                    Number(this.genericConfigDraft.revoke_after_days)
            ) {
                return "Ngày khóa cụm sân không được lớn hơn ngày thu hồi quyền.";
            }
            if (
                this.configurationType === "account_policy" &&
                Number(this.genericConfigDraft.warnings_to_restrict) >
                    Number(this.genericConfigDraft.violations_to_lock)
            ) {
    ...
    watch: {
        ...
        activeTab(v) {
            this.$router
                .replace({ query: { ...this.$route.query, tab: v } })
                .catch(() => {});
        },
    },
    async mounted() {
    ...
    methods: {
        ...
        async loadDetail() {
            ...
            this.error = "";
            try {
                const res = await adminPolicyService.show(
        ...
        async loadOptions() {
            ...
            try {
                ...
                const tData = t.data;
                this.ruleTemplates = Array.isArray(tData)
                    ? tData
                    : tData
                      ? Object.values(tData)
                      : [];
        ...
        async clonePolicy() {
            this.saving = true;
            try {
                const res = await adminPolicyService.cloneVersion(
                    this.policy.id,
                );
                this.$router.push({
        ...
        async archivePolicy() {
            this.saving = true;
            try {
                const res = await adminPolicyService.updateStatus(
                    this.policy.id,
                    { status: "archived", reason: "Ngưng áp dụng." },
                );
        ...
        async saveContent() {
            ...
            try {
                ...
                const res = await adminPolicyService.update(p.id, {
                    ...
                    is_overridable: !!p.is_overridable,
        ...
        openCancelRefundModal() {
            ...
            this.cancelRefundDraft = JSON.parse(JSON.stringify(tiers)).map(
                (tier, index) => ({
            ...
            this.ownerFaultFullRefundDraft =
                this.ownerFaultFullRefund.enabled !== false;
        ...
        addCancelRefundTier() {
            ...
            if (!last) {
                this.cancelRefundDraft = [
                    this.defaultCancelRefundTier(0, null, true, 0),
                ];
                return;
            }
            ...
            sorted.push(
                this.defaultCancelRefundTier(
                    splitAt,
                    null,
                    true,
                    Number(last.refund_percent || 0),
                ),
            );
        ...
        async saveCancelRefundConfig() {
            ...
            try {
                await adminPolicyService.saveCancelRefundTiers(this.policy.id, {
                    owner_fault_full_refund_enabled:
                        !!this.ownerFaultFullRefundDraft,
        ...
        openReportModal() {
            if (!this.canEditConfig) return;
            this.reportThresholdDraft = (
                this.reportThresholdRows.length
                    ? this.reportThresholdRows
                    : [this.defaultReportThreshold()]
            ).map((threshold, index) =>
                this.normalizeThresholdShape(threshold, index),
            );
        ...
        removeReportThreshold(key) {
            ...
            this.reportThresholdDraft = this.reportThresholdDraft.filter(
            ...
            try {
                ...
                if (this.configurationType === "platform_fee") {
                    ...
                    configurationData.message_template =
                        "Cụm sân của bạn đã đến hoặc quá hạn phí nền tảng.";
                ...
                await adminPolicyService.updatePolicyConfiguration(
                    this.policy.id,
                    {
        ...
        defaultReportThreshold() {
            ...
            const action =
                this.reportActionOptionsFor(objectType)[0]?.value ||
                "notify_admin";
            return {
                ...
                action,
                action_label:
                    this.reportActionOptionsFor(objectType)[0]?.label || action,
                notify_admin: true,
                notify_reported_user: false,
                is_active: true,
            };
        ...
        normalizeThresholdShape(threshold, index = 0) {
            const objectType =
                threshold.object_type || threshold.target_type || "post";
            const actions = this.reportActionOptionsFor(objectType);
            const action =
                threshold.action || actions[0]?.value || "notify_admin";
            return {
                key: threshold.key || `threshold_${index}_${Date.now()}`,
                object_type: objectType,
                object_type_label:
                    threshold.object_type_label ||
                    this.targetTypeLabel(objectType),
                min_reports: Number(
                    threshold.min_reports || threshold.minimum_reports || 1,
                ),
                min_distinct_reporters: Number(
                    threshold.min_distinct_reporters ||
                        threshold.minimum_unique_reporters ||
                        1,
                ),
                within_days: Number(
                    threshold.within_days || threshold.window_days || 1,
                ),
                action,
                action_label:
                    threshold.action_label ||
        ...
        normalizeThresholdAction(threshold) {
            ...
            threshold.object_type_label = this.targetTypeLabel(
                threshold.object_type,
            );
            threshold.action_label =
                options.find((option) => option.value === threshold.action)
                    ?.label || threshold.action;
        },
        ...
        reportActionOptionsFor(objectType) {
            ...
            return (
                ...
                options.post || [
                    ...
                    {
                        value: "mark_pending_review",
                        label: "Đưa vào chờ duyệt",
                    },
        ...
        dynamicTierLabel(tier) {
            ...
            const to =
                ...
                tier.to_hours === undefined
                    ...
                    : Number(tier.to_hours);
            if (from === 0 && to !== null)
                return `Trong ${to} giờ trước giờ chơi`;
        ...
        cancelRefundResult(tier) {
            ...
            return percent > 0
                ? `Cho hủy, hoàn ${percent}%`
                : "Cho hủy, không hoàn tiền";
        },
        ...
        goStep2() {
            if (!this.ruleForm.action_code) {
                this.wizardError = "Vui lòng chọn tình huống.";
                return;
            }
            this.wizardError = "";
        ...
        buildPayload(active) {
            ...
            if (t === "cancel_before_hours") {
                condition.hours_before_start = { gte: +n.hours_before_start };
                result.can_cancel = true;
                decisionKey = decisionKey || "can_cancel";
            }
            if (t === "refund_percent_by_cancel_time") {
                condition.hours_before_start = { gte: +n.hours_before_start };
            ...
            if (t === "platform_fee_overdue_lock") {
                condition.overdue_days = { gte: +n.overdue_days };
                result.action = "limit_owner_access";
                result.access_mode = "limited";
                decisionKey = decisionKey || "owner_access_mode";
            }
            if (t === "report_threshold_requires_review") {
                condition.report_count = { gte: +n.report_count };
                condition.unique_reporters = { gte: +n.unique_reporters };
                condition.window_days = +n.window_days;
                result.action = "mark_pending_review";
                decisionKey = decisionKey || "moderation_action";
            }
            if (t === "contract_signing_required") {
        ...
        async saveRule(active) {
            ...
            try {
                ...
                const res = await adminPolicyService.addRule(
                    this.policy.id,
                    this.buildPayload(active),
                );
        ...
        async toggleRule(rule) {
            try {
                const res = await adminPolicyService.toggleRule(
                    this.policy.id,
                    rule.id,
                );
...
.policy-status-text.status-draft {
    color: var(--admin-blue);
}
.policy-status-text.status-active {
    color: var(--admin-success-text);
}
.policy-status-text.status-inactive,
.policy-status-text.status-archived {
    color: var(--admin-muted);
}
.policy-status-text.status-rejected {
    color: var(--admin-danger-text);
}
.policy-status-text.status-pending {
...
/* ── Status text in the content remains compact, never a pill ── */
.badge {
    display: inline;
    padding: 0;
    border-radius: 0;
...
.content-textarea:not([readonly]):focus {
    outline: none;
    border-color: #16a34a;
    background: #fff;
}
...
.rule-row.never-hover-class-placeholder {
...
.btn.never-hover-class-placeholder {
...
.btn.danger-ghost.never-hover-class-placeholder {
    background: #fff1f2;
}
...
.option-card.never-hover-class-placeholder {
    border-color: #86efac;
}
...
.link-btn.never-hover-class-placeholder {
    text-decoration: underline;
}
...
.name-fields .param-field input:focus {
    outline: none;
    border-color: #16a34a;
}
...
.config-edit-grid.report-grid {
    grid-template-columns: 1.2fr repeat(4, minmax(100px, 1fr)) repeat(
            3,
            auto
        ) auto;
}
.config-edit-grid.generic-grid {
...
.preview-banner.invalid p:last-child {
    color: #dc2626;
}
