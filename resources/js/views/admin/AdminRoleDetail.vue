<template>
    <section class="admin-page role-detail-page">
        ...
        <template v-if="!loading && role">
            <header class="role-detail-context">
                ...
                <button class="role-back-btn" type="button" @click="$router.push({ name: 'admin-roles' })">
            ...
            >
                <div class="section-head with-search">
                    <div>
                        ...
                        <p>
                            Bật Truy cập trước, sau đó mới cấp Thêm, Sửa,
                            Xóa/Ngưng hoặc Duyệt/Xử lý.
                        </p>
                    </div>
                    <input
                ...
                <div class="permission-rule">
                    ...
                    <span
                        >Có quyền thao tác thì bắt buộc có Truy cập. Khi tắt
                        Truy cập, các quyền phụ thuộc sẽ tự được thu hồi.</span
                    >
                </div>
                    ...
                    <table class="permission-table">
                        <thead>
                            <tr>
                                ...
                                <th
                                    v-for="column in permissionActionColumns"
                                    :key="column.key"
                                    class="action-column"
                                >
                                    {{ column.label }}
                        ...
                        <tbody>
                            ...
                            >
                                <tr class="permission-group-row">
                                    <td
                                        :colspan="
                                            permissionActionColumns.length + 2
                                        "
                                    >
                                        <strong>{{
                                            group.module_label ||
                                            getModuleMeta(group.group_name)
                                                .label
                                        }}</strong>
                                        <span>{{
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            ...
                                            "
                                            :title="
                                                actionTitle(row, column.key)
                                            "
                                            @click="
                                                toggleAction(row, column.key)
                                            "
                                        >
                                            <span></span>
                                        </button>
            ...
            <section v-if="activeTab === 'users'" class="panel">
                <div class="section-head">
                    ...
                    <p>
                        Danh sách tối đa 100 tài khoản đang được gán nhóm quyền
                        này.
                    </p>
                </div>
            ...
            <section v-if="activeTab === 'audit'" class="panel">
                <div class="section-head">
                    ...
                    <p>
                        Hiển thị theo dòng thời gian để dễ kiểm tra ai đã sửa
                        nhóm quyền.
                    </p>
                </div>
                ...
                <div v-else class="timeline">
                    ...
                    >
                        ...
                        <div>
                            ...
                            <div
                                v-if="auditDiffs(log).length"
                                class="diff-list"
                            >
                                <div
                                    v-for="diff in auditDiffs(log)"
                                    :key="diff.field"
                                    class="diff-row"
                                >
                                    ...
                                    <template v-else>
                                        <strong>{{
                                            diff.field_label || diff.fieldLabel
                                        }}</strong>
                                        <span>{{
                                            formatDisplayValue(
                                                diff.old || diff.oldLabel,
                                            )
                                        }}</span>
                                        <span>→</span>
                                        <span>{{
                                            formatDisplayValue(
                                                diff.new || diff.newLabel,
                                            )
                                        }}</span>
                                    </template>
                                </div>
...
export default {
    ...
    computed: {
        hasPermissionChanges() {
            return (
                this.sortedIds(this.selectedPermissionIds).join(",") !==
                this.sortedIds(this.originalPermissionIds).join(",")
            );
        },
        canUpdateRole() {
            return (
        ...
        canEditPermissionMatrix() {
            return (
                Boolean(this.role?.can_edit_permissions) &&
    ...
    methods: {
        ...
        async loadDetail() {
            ...
            this.error = "";
            try {
                const response = await adminRoleService.show(
                ...
                this.selectedPermissionIds = (data.permissions || []).map(
                ...
                this.error =
                    error.message || "Không tải được chi tiết nhóm quyền.";
            } finally {
                this.loading = false;
            }
        },
        ...
        async savePermissions() {
            await this.runAction(
                () =>
                    adminRoleService.updatePermissions(
                        this.role.id,
        ...
        hasPermission(permissionId) {
            return this.selectedPermissionIds.some(
        ...
        isActionDisabled(row, actionKey) {
            ...
            return (
                actionKey !== "access" && !this.isActionEnabled(row, "access")
            );
        },
        ...
        addPermissionIds(ids) {
            this.selectedPermissionIds = this.sortedIds([
                ...new Set([
                    ...this.selectedPermissionIds.map(Number),
                    ...ids.map(Number),
                ]),
            ]);
        },
        ...
        auditDiffs(log) {
            return log.changes_summary?.length
                ? log.changes_summary
...
.role-detail-title .eyebrow {
    margin: 0;
}

.role-detail-title h2 {
    margin: 0;
    font-size: 22px;
    font-weight: 400;
}
...
.role-detail-page > .tabs button.active {
    border-color: #16a34a;
    background: #16a34a;
    color: #fff;
}
