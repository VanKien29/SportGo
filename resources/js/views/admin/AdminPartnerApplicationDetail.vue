<template>
    <div class="partner-detail-page">
        ...
        <section v-else-if="!application" class="state-card error">
        ...
        <template v-else>
            <section class="action-strip">
                ...
                <div class="action-buttons">
                    <button
                        ...
                        class="btn primary"
                        type="button"
                        @click="openReviewAction('approve')"
                    >
                    ...
                    <details v-if="canReviewApplication" class="review-more">
                        ...
                        <div class="review-more-menu">
                            <button
                                type="button"
                                @click="openReviewAction('supplement')"
                            >
                                <AppIcon name="fileText" size="15" /> Yêu cầu bổ
                                sung
                            </button>
                            <button
                                class="danger"
                                type="button"
                                @click="openReviewAction('reject')"
                            >
            ...
            <section
                class="partner-summary-rail"
                aria-label="Tóm tắt hồ sơ đối tác"
            >
                <div class="summary-stat">
                    <span>Mã đối tác</span>
                    <strong>{{
                        application.partner_summary?.partner_code ||
                        `PTN-${String(application.user_id || "")
                            .slice(0, 8)
                            .toUpperCase()}`
                    }}</strong>
                </div>
                <div class="summary-stat">
                    <span>Số hồ sơ</span>
                    <strong>{{
                        application.partner_summary?.application_count || 1
                    }}</strong>
                </div>
                <div class="summary-stat">
                    <span>Cụm sân quản lý</span>
                    <strong>{{
                        application.partner_summary?.managed_clusters_count ||
                        (application.approved_venue_cluster_id ? 1 : 0)
                    }}</strong>
                </div>
                <div class="summary-stat">
                    <span>Sân đã cấu hình</span>
                    <strong>{{
                        application.courts_count ||
                        application.courts?.length ||
                        0
                    }}</strong>
                </div>
            </section>
            ...
            <section
                v-if="
                    ...
                    !pendingSportgoDocument &&
                    application.status !== 'completed'
                "
                class="review-panel readonly-panel"
            >
            ...
            <section
                v-if="actionMode === 'approve' && canReviewApplication"
                class="review-panel"
            >
                <div class="panel-head">
                    <div>
                        ...
                        <p>
                            Sau khi duyệt, hệ thống tạo giấy/hợp đồng đối tác và
                            chờ SportGo ký trước.
                        </p>
                    </div>
                    <button
                        class="icon-btn"
                ...
                <form class="review-form" @submit.prevent="submitApprove">
                    <label
                        v-if="requiresInitialCourt"
                        class="field"
                        :class="{ invalid: fieldErrors.initial_court_name }"
                    >
                        <span>Tên sân con ban đầu</span>
                        <input
                            v-model.trim="approveForm.initial_court_name"
                            type="text"
                            placeholder="Ví dụ: Sân 1"
                        />
                        <small v-if="fieldErrors.initial_court_name">{{
                            fieldErrors.initial_court_name
                        }}</small>
                    </label>
                    ...
                    >
                        ...
                        <small v-if="fieldErrors.court_type_id">{{
                            fieldErrors.court_type_id
                        }}</small>
                    </label>
                    ...
                    <label class="field full">
                        ...
                        <textarea
                            v-model.trim="approveForm.review_note"
                            rows="4"
                            placeholder="Ghi chú nội bộ nếu cần"
                        ></textarea>
                    </label>

                    <p v-if="actionError" class="inline-error full">
                        {{ actionError }}
                    </p>
            ...
            <section
                v-if="
                    (actionMode === 'reject' || actionMode === 'supplement') &&
                    canReviewApplication
                "
                class="review-panel"
                :class="{ 'danger-panel': actionMode === 'reject' }"
            >
                <div class="panel-head">
                    <div>
                        <h3>
                        ...
                        <p>
                            {{
                                actionMode === "supplement"
                                    ? "Nhập rõ giấy tờ/thông tin cần bổ sung. Nội dung này sẽ được gửi cho người đăng ký qua email."
                                    : "Bắt buộc nhập lý do. Lý do này sẽ được gửi cho người đăng ký qua email."
                            }}
                        </p>
                    </div>
                    <button
                ...
                <form class="review-form" @submit.prevent="submitReject">
                    <label
                        class="field full"
                        :class="{ invalid: fieldErrors.reason }"
                    >
                        <span>{{
                            actionMode === "supplement"
                        ...
                        <textarea
                            ...
                            :placeholder="
                                actionMode === 'supplement'
                                    ? 'Ví dụ: bổ sung ảnh mặt sau CCCD, hợp đồng thuê mặt bằng còn hiệu lực...'
                                    : 'Nêu rõ lý do hồ sơ chưa đạt'
                            "
                        ></textarea>
                        <small v-if="fieldErrors.reason">{{
                            fieldErrors.reason
                        }}</small>
                    </label>
            ...
            <div v-if="detailsLoading" class="detail-loading" role="status">
                <AppIcon name="refresh" size="16" /> Đang tải dữ liệu chi
                tiết...
            </div>
            ...
            <section v-if="activeTab === 'overview'" class="summary-grid">
                <InfoPanel
                    ...
                    :items="[
                        ...
                        [
                            'Ngày sinh',
                            dateOnly(application.applicant_birth_date),
                        ],
                ...
                <InfoPanel
                    title="Giấy tờ và pháp lý"
                    :items="[
                        ['Người đại diện', application.representative_name],
                        [
                            'Loại giấy tờ',
                            identityLabel(
                                application.representative_identity_type,
                            ),
                        ],
                        [
                            'Số giấy tờ',
                        ...
                        [
                            'Ngày cấp',
                            dateOnly(
                                application.representative_identity_issued_date,
                            ),
                        ],
                        [
                            'Nơi cấp',
                            application.representative_identity_issued_place,
                        ],
                ...
                <InfoPanel
                    ...
                    :items="[
                        ...
                        [
                            'Loại đăng ký',
                            applicantTypeLabel(application.applicant_type),
                        ],
                    ]"
                />
                    ...
                    :items="[
                        ...
                        [
                            ...
                            bankVerificationLabel(
                                application.bank_verification_status,
                            ),
                        ],
                    ]"
                />
                ...
                <InfoPanel
                    ...
                    :items="[
                        ...
                        [
                            'Giờ mở cửa',
                            application.expected_opening_hours || '-',
                        ],
                        ['Tiện ích', compactList(application.amenities)],
                        [
                            'Số sân con',
                            application.court_count_total ||
                                application.courts_count ||
                                0,
                        ],
            ...
            <section v-if="activeTab === 'courts'" class="content-card">
                <div class="card-head">
                    ...
                    <span
                        >{{ application.partner_applications?.length || 0 }} cụm
                        sân</span
                    >
                </div>
                ...
                <div class="cluster-list">
                    <article
                        v-for="item in application.partner_applications || []"
                        :key="item.id"
                        class="cluster-row"
                        :class="{ current: item.id === application.id }"
                    >
                        <div>
                            ...
                            <small
                                ...
                                {{
                                    contractStatusLabel(item.contract_status)
                                }}
                                · {{ item.courts_count || 0 }} sân</small
                            >
                        </div>
                        <button
                            class="btn ghost small"
                            type="button"
                            :disabled="item.id === application.id"
                            @click="openSiblingApplication(item)"
                        >
                            Xem hồ sơ
                        </button>
                    </article>
                ...
                <div class="court-grid">
                    <article
                        v-for="court in application.courts || []"
                ...
                <p v-if="!application.courts?.length" class="empty-text">
                    Hồ sơ chưa có sân con.
                </p>
            </section>
            ...
            <section v-if="activeTab === 'documents'" class="documents-grid">
                    ...
                    <div class="doc-list">
                        ...
                        >
                            <div class="doc-group-head">
                                ...
                                <span
                                    >{{ group.documents.length }} văn bản</span
                                >
                            </div>
                            <div
                            ...
                            >
                                <div>
                                    ...
                                    <p
                                        v-if="document.file_available === false"
                                        class="doc-file-warning"
                                    >
                                        File gốc không còn trên hệ thống. Không
                                        thể xem hoặc tải xuống.
                                    </p>
                                </div>
                                <div class="row-actions">
                                    <button
                                        class="btn primary small icon-only"
                                        :title="
                        ...
                        <p v-if="!generatedDocuments.length" class="empty-text">
                            Chưa có văn bản hệ thống.
                        </p>
                    </div>
                </article>
                ...
                <article class="content-card">
                    ...
                    <div class="doc-list">
                        <div
                            v-for="group in uploadedDocumentGroups"
                            :key="group.key"
                            class="doc-group"
                        >
                            <div class="doc-group-head">
                            ...
                            >
                                <div>
                                    ...
                                    <p
                                        ...
                                        class="doc-file-warning"
                                    >
                                        File đã tải lên không còn trên hệ thống.
                                        Vui lòng yêu cầu đối tác bổ sung lại.
                                    </p>
                                </div>
                                <div class="row-actions">
                                    <button
                                        ...
                                        "
                                        @click="
                                            openDocument(document, 'uploaded')
                                        "
                                    >
                                        <AppIcon name="eye" size="15" />
                                    </button>
                                    <button
                                        ...
                                        "
                                        @click="
                                            downloadUploadedDocument(document)
                                        "
                                    >
                                        <AppIcon name="download" size="15" />
                                    </button>
                        ...
                        <p v-if="!uploadedDocuments.length" class="empty-text">
                            Chưa có tài liệu phụ lục.
                        </p>
                    </div>
                </article>
            </section>
            ...
            <section v-if="activeTab === 'signing'" class="content-card">
                <div class="card-head">
                    <div>
                        ...
                        <p class="muted">
                            OTP chỉ áp dụng cho bước xác nhận của chủ sân; admin
                            ký trực tiếp bằng tài khoản đang đăng nhập.
                        </p>
                    </div>
                ...
                <div class="signing-log-list">
                    <article
                        v-for="log in signingLogs"
                        :key="log.id"
                        class="signing-log-row"
                    >
                        <div>
                            <strong>{{ log.document_title }}</strong>
                            <p>
                                {{ signingSideLabel(log.signer_side) }} ·
                                {{ log.otpStatusLabel }} ·
                                {{ log.otp_identifier || "-" }}
                            </p>
                            <dl>
                    ...
                    <p v-if="!signingLogs.length" class="empty-text">
                        Chưa có giao dịch chữ ký nào.
                    </p>
                </div>
            </section>
            ...
            <section v-if="activeTab === 'history'" class="content-card">
                ...
                <div class="timeline">
                    <article
                        v-for="item in application.status_histories || []"
                        :key="`${item.new_status}-${item.created_at}`"
                        class="timeline-row"
                    >
                        <span></span>
                        <div>
            ...
            <section v-if="activeTab === 'settlement'" class="content-card">
                <div class="card-head">
                    ...
                    <div class="head-actions">
                        <span
                            >{{ activeTerminationRequests.length }} đang xử
                            lý</span
                        >
                        <button
                            ...
                            type="button"
                            @click="
                                showClosedTerminations = !showClosedTerminations
                            "
                        >
                            {{
                                showClosedTerminations
                                    ? "Ẩn lịch sử"
                                    : `Lịch sử (${closedTerminationRequests.length})`
                            }}
                        </button>
                        <button
                        ...
                        >
                            <AppIcon name="xCircle" size="14" /> Đơn phương chấm
                            dứt
                        </button>
                    </div>
                </div>
                ...
                <div class="termination-list">
                    <article
                        v-for="request in displayedTerminationRequests"
                        :key="request.id"
                        class="termination-card"
                    >
                        <div class="term-header">
                            <div>
                                <strong>{{
                                    request.termination_code ||
                                    "Yêu cầu chấm dứt"
                                }}</strong>
                                <span
                                    class="badge"
                                    ...
                                    >{{
                                        terminationStatusLabel(request.status)
                                    }}</span
                                >
                            </div>
                        </div>
                        ...
                        <div class="termination-next-action">
                            <strong>{{
                                terminationNextAction(request).title
                            }}</strong>
                            <p>
                                {{ terminationNextAction(request).description }}
                            </p>
                            <div class="termination-task-actions">
                                ...
                                >
                                    <AppIcon name="check" size="14" /> Xác nhận
                                    yêu cầu
                                </button>
                                <button
                                    v-if="
                                        isUnilateralNotice(request) &&
                                        isTerminationDraftStatus(request.status)
                                    "
                                    class="btn primary small"
                                    type="button"
                                    :disabled="saving"
                                ...
                                >
                                    <AppIcon name="pencil" size="14" /> Xem và
                                    ký công văn
                                </button>
                                <button
                            ...
                            <div class="termination-progress">
                                <span
                                    v-for="step in terminationAdminSteps(
                                        request,
                                    )"
                                    :key="step.key"
                                    :class="step.state"
                        ...
                        <div class="termination-finance-grid">
                            <div>
                                <span class="label">Booking tương lai</span
                                ><strong>{{
                                    terminationFinancial(
                                        request,
                            ...
                            <div>
                                <span class="label">Có thể rút</span
                            ...
                            <div>
                                <span class="label">Nợ booking online</span
                            ...
                            <div>
                                <span class="label">Hoàn/rút tiền treo</span
                                ><strong>{{
                                    money(
                                        Number(
                                            terminationFinancial(
                        ...
                        <form
                            ...
                            @submit.prevent="
                                submitTerminationInlineAction(request)
                            "
                        >
                            <label class="field">
                                <span>{{
                                    terminationInlineAction.type === "withdraw"
                                        ? "Lý do thu hồi công văn"
                                        : "Phản hồi giữ nguyên công văn"
                                }}</span>
                                <textarea
                                    v-model.trim="terminationInlineAction.note"
                                    rows="3"
                                    minlength="10"
                                    maxlength="1000"
                                    required
                            ...
                            <div class="actions compact">
                                <button
                                    class="btn ghost small"
                                    type="button"
                                    @click="closeTerminationInlineAction"
                                >
                                    Đóng
                                </button>
                                <button
                                    ...
                                    :disabled="
                                        saving ||
                                        terminationInlineAction.note.length < 10
                                    "
                                >
                                    {{
                        ...
                        <div class="termination-actions">
                            ...
                            >
                                <AppIcon name="fileText" size="14" /> Xác nhận
                                thủ công & sinh biên bản
                            </button>
                            <button
                                v-if="
                                    isTerminationFinalSignatureStatus(
                                        request.status,
                                    )
                                "
                                class="btn primary small"
                            ...
                            >
                                ...
                                {{
                                    terminationFinalAdminSigned(request)
                                        ? "Xem biên bản cuối"
                                        : "Mở file và ký biên bản"
                                }}
                            </button>
                        </div>

                        <details class="termination-detail-disclosure">
                            <summary>
                                <span
                                    >Chi tiết hồ sơ, booking và quyết toán</span
                                >
                                <small
                                    >{{ request.documents?.length || 0 }} văn
                                    bản</small
                                >
                            </summary>
                            <div class="termination-detail-body">
                                <div class="term-info-grid">
                                    ...
                                    <div>
                                        <span class="label"
                                            >Ngày thu hồi quyền</span
                                        ><span>{{
                                            formatDate(
                                                request.transition_end_at,
                                            )
                                        }}</span>
                                    </div>
                                    <div v-if="isUnilateralNotice(request)">
                                        <span class="label"
                                            >Chủ sân xác nhận nhận</span
                                        ><span>{{
                                            formatDate(
                                                request.workflow_state
                                ...
                                <div
                                    v-if="request.booking_actions?.length"
                                    class="settlement-box"
                                >
                                    <h4>Booking tương lai</h4>
                                    <div
                                        v-for="action in request.booking_actions"
                                        :key="action.id"
                                    ...
                                    >
                                        ...
                                        <span>{{
                                            bookingActionLabel(action.action)
                                        }}</span>
                                        <span
                                            class="badge"
                                            :class="`badge-${action.status}`"
                                            >{{ action.status }}</span
                                        >
                                        <button
                                ...
                                >
                                    ...
                                    <table
                                        v-if="request.settlement.items?.length"
                                    ...
                                    >
                                        <h5>Lệnh rút tiền</h5>
                                        <div
                                            v-for="wd in request.settlement
                                                .withdrawal_requests"
                                            :key="wd.id"
                                            class="withdrawal-row"
                                        >
                    ...
                    <p
                        v-if="!displayedTerminationRequests.length"
                        class="empty-text"
                    >
                        {{
                            closedTerminationRequests.length
                                ? "Không có hồ sơ chấm dứt đang xử lý. Mở lịch sử để xem hồ sơ đã kết thúc."
                                : "Chưa có yêu cầu chấm dứt hoặc quyết toán."
                        }}
                    </p>
                </div>
            </section>
            ...
            >
                <form
                    class="modal-panel"
                    @submit.prevent="handleUnilateralTermination"
                >
                    ...
                    <div class="modal-body">
                        <p class="muted">
                            Bước này chỉ tạo bản xem trước. Công văn chưa gửi và
                            cụm sân chưa bị khóa cho đến khi admin xem file, ký
                            và xác nhận phát hành.
                        </p>
                        <label class="field">
                            <span>Lý do chấm dứt</span>
                            <textarea
                                v-model.trim="terminationModal.reason"
                                rows="4"
                                required
                                maxlength="2000"
                        ...
                        <label class="field">
                            ...
                            <textarea
                                v-model.trim="terminationModal.detail_reason"
                                rows="4"
                                maxlength="5000"
                                placeholder="Nêu căn cứ, sự kiện và dữ liệu cần đối tác biết"
                            ></textarea>
                        </label>
                        <div class="modal-form-grid">
                            <label class="field">
                                ...
                                <input
                                    v-model="
                                        terminationModal.requested_effective_date
                                    "
                                    type="date"
                                    required
                                />
                            ...
                            <label class="field">
                                ...
                                <select
                                    v-model="
                                        terminationModal.future_booking_policy
                                    "
                                    required
                                >
                                    <option value="manual_per_booking">
                                        Xử lý thủ công từng booking
                                    </option>
                                    <option value="serve_until_last_booking">
                                        Tiếp tục phục vụ đến booking cuối
                                    </option>
                                    <option
                                        value="cancel_all_refund_to_user_balance"
                                    >
                                        Hủy và hoàn tiền toàn bộ
                                    </option>
                                </select>
                            </label>
                        </div>
        ...
        <DocumentViewerModal
            :show="unilateralNoticeModal.open"
            :document="unilateralNoticeModal.document"
            action-mode
            @close="closeUnilateralNotice"
        >
            <template #actions>
                <div class="unilateral-sign-panel">
                    <div class="admin-signing-context">
                        <strong>Công văn chấm dứt hợp tác</strong>
                        <p class="muted">
                            Kiểm tra file, ký vào khung và xác nhận phát hành
                            bằng tài khoản admin đang đăng nhập.
                        </p>
                    </div>
                    <div class="admin-signature-field">
                        <div>
                            <strong>Chữ ký đại diện SportGo</strong>
                            <button
                                type="button"
                                @click="prepareUnilateralNoticeSignature"
                            >
                                Xóa
                            </button>
                        </div>
                    ...
                    <label class="admin-sign-confirm">
                        <input
                            v-model="unilateralNoticeModal.accepted"
                            type="checkbox"
                        />
                        <span
                            >Tôi xác nhận đại diện SportGo đã kiểm tra đúng văn
                            bản đang hiển thị và đồng ý phát hành.</span
                        >
                    </label>
                    <div class="actions compact admin-sign-submit">
                        <button
                            class="btn primary small"
                            type="button"
                            :disabled="
                                saving ||
                                unilateralNoticeSignatureEmpty ||
                                !unilateralNoticeModal.accepted
                            "
                            @click="signAndIssueUnilateralNotice"
                        >
                            {{
                                saving
                                    ? "Đang phát hành..."
                                    : "Ký và phát hành công văn"
                            }}
                        </button>
                    </div>
                </div>
        ...
        <DocumentViewerModal
            :show="terminationFinalModal.open"
            :document="terminationFinalModal.document"
            :action-mode="
                !!terminationFinalModal.request &&
                !terminationFinalAdminSigned(terminationFinalModal.request)
            "
            @close="closeTerminationFinalModal"
        >
            <template #actions>
                <div
                    v-if="
                        terminationFinalModal.request &&
                        !terminationFinalAdminSigned(
                            terminationFinalModal.request,
                        )
                    "
                    class="unilateral-sign-panel"
                >
                    <div class="admin-signing-context">
                        <strong>Biên bản chấm dứt cuối</strong>
                        <p class="muted">
                            SportGo ký bằng tài khoản admin hiện tại; chủ sân
                            chỉ được ký sau khi chữ ký này được lưu vào file.
                        </p>
                    </div>
                    <div class="admin-signature-field">
                        <div>
                            ...
                            <button
                                type="button"
                                @click="clearTerminationFinalSignature"
                            >
                                Xóa
                            </button>
                        </div>
                    ...
                    <label class="admin-sign-confirm">
                        <input
                            v-model="terminationFinalSigning.accepted"
                            type="checkbox"
                        />
                        <span
                            >Tôi xác nhận đại diện SportGo đã kiểm tra đúng biên
                            bản đang hiển thị và đồng ý ký.</span
                        >
                    </label>
                    <div class="actions compact admin-sign-submit">
                        <button
                            ...
                            :disabled="
                                saving ||
                                terminationFinalSignatureEmpty ||
                                !terminationFinalSigning.accepted
                            "
                            @click="
                                handleSignTerminationFinalDocument(
                                    terminationFinalModal.request,
                                )
                            "
                        >
                            {{ saving ? "Đang lưu chữ ký..." : "Ký biên bản" }}
                        </button>
                    </div>
                </div>
            </template>
...
const activeTab = ref(
    detailTabs.includes(String(route.query.tab))
...
const terminationInlineAction = reactive({
    requestId: null,
    type: "",
    note: "",
});
...
const terminationFinalModal = reactive({
    open: false,
    request: null,
    document: null,
});
const terminationFinalSigning = reactive({ accepted: false });
...
const generatedDocuments = computed(() =>
    (application.value?.documents || []).filter((d) => d.source !== "uploaded"),
);
const uploadedDocuments = computed(
    () => application.value?.uploaded_documents || [],
);
const generatedDocumentGroups = computed(() =>
    groupDocuments(generatedDocuments.value, generatedDocumentGroupMeta),
);
const uploadedDocumentGroups = computed(() =>
    groupDocuments(uploadedDocuments.value, uploadedDocumentGroupMeta),
);
const signingLogs = computed(() =>
    generatedDocuments.value.flatMap((document) =>
        (document.signing_requests || []).map((log) => ({
            ...log,
            document_title:
                document.title || documentTypeLabel(document.document_type),
            otpStatusLabel: signingStatusLabel(log.status || log.otp_status),
        })),
    ),
);
const requiresInitialCourt = computed(
    () => !(application.value?.courts || []).length,
);
const leafCourtTypes = computed(() =>
    courtTypes.value.filter(
        (type) =>
            type.is_active !== false && Number(type.children_count || 0) === 0,
    ),
);
const pendingSportgoDocument = computed(
    () =>
        generatedDocuments.value.find(
            (document) =>
                [
                    "partner_contract",
...
const canReviewApplication = computed(() =>
    isReviewable(application.value?.status),
);
const activeContract = computed(
    () =>
        (application.value?.contracts || []).find(
            (c) => c.status === "signed_active",
        ) || null,
);
const activeTerminationRequests = computed(() =>
    (application.value?.termination_requests || []).filter((request) =>
        TERMINATING_STATUSES.includes(request.status),
    ),
);
const closedTerminationRequests = computed(() =>
    (application.value?.termination_requests || []).filter(
        (request) => !TERMINATING_STATUSES.includes(request.status),
    ),
);
const displayedTerminationRequests = computed(() =>
    showClosedTerminations.value
        ? [
              ...
              ...closedTerminationRequests.value,
          ]
        : activeTerminationRequests.value,
);
const pendingTerminationRequest = computed(
    () => activeTerminationRequests.value[0] || null,
);
const readonlyActionTitle = computed(() => {
    if (application.value?.status === "need_supplement")
        return "Đang chờ người dùng bổ sung";
...
const readonlyActionText = computed(() => {
    if (application.value?.status === "need_supplement")
        return "Admin đã gửi yêu cầu bổ sung. Chỉ xử lý tiếp sau khi người dùng cập nhật hồ sơ và ký lại đơn đăng ký.";
    if (application.value?.status === "rejected")
        return "Hồ sơ đã dừng ở trạng thái từ chối. Người dùng có thể tạo bản sao hồ sơ để đăng ký lại.";
    if (application.value?.status === "cancelled")
        return "Hồ sơ đã bị hủy và không thể duyệt.";
    if (application.value?.status === "completed")
        return "Hồ sơ đã hoàn tất ký hợp đồng và kích hoạt đối tác.";
...
onMounted(async () => {
    ...
    if (route.query.action || activeTab.value !== "overview")
...
watch(
    ...
    (action) => {
        ...
        if (action && action !== "overview") ensureDetailsLoaded();
    },
);
...
watch(
    () => route.query.tab,
    (tab) => {
        if (detailTabs.includes(String(tab))) selectTab(String(tab));
    },
);
...
async function loadApplication() {
    ...
    try {
        const response = await adminPartnerApplicationService.show(
            route.params.id,
            { view: "summary" },
        );
        ...
        if (
            !isReviewable(application.value?.status) &&
            ["approve", "reject", "supplement"].includes(actionMode.value)
        ) {
            actionMode.value = "";
        }
        await nextTick();
...
async function ensureDetailsLoaded() {
    if (detailsLoaded.value || detailsLoading.value || !application.value?.id)
        return;
    ...
    try {
        ...
        error.value = err.message || "Không tải được dữ liệu chi tiết hồ sơ.";
    } finally {
...
async function selectTab(tab) {
    activeTab.value = tab;
    if (tab !== "overview") await ensureDetailsLoaded();
}

async function openReviewAction(action) {
    await ensureDetailsLoaded();
...
function openDocument(doc, type = "generated") {
    ...
    const documentApplicationId =
        doc?.partner_application_id || application.value?.id;
...
async function downloadGeneratedDocument(document) {
    ...
    if (document.file_available === false) {
        error.value =
            "File văn bản không còn trên hệ thống nên chưa thể tải xuống.";
        return;
    }
    try {
...
async function downloadUploadedDocument(document) {
    ...
    try {
        await adminPartnerApplicationService.downloadUploadedDocument(
            document.id,
        );
    } catch (err) {
...
async function submitApprove() {
    ...
    saving.value = true;
    try {
        const response = await adminPartnerApplicationService.approve(
            application.value.id,
            {
...
async function submitReject() {
    ...
    saving.value = true;
    try {
        const response = await adminPartnerApplicationService.reject(
            application.value.id,
            {
...
function isTerminationSubmittedStatus(status) {
    return terminationStatusIn(status, [
        "submitted",
        "cancellation_in_progress",
    ]);
}
...
function canConfirmTerminationRequest(request) {
    return (
        isTerminationSubmittedStatus(request.status) &&
        request.termination_type !== "unilateral_by_sportgo"
    );
}
...
function hasPendingReconsideration(request) {
    return Boolean(
        isUnilateralNotice(request) &&
        request.workflow_state?.reconsideration_pending,
    );
}
...
function canWithdrawUnilateralNotice(request) {
    return Boolean(
        isUnilateralNotice(request) &&
        ["draft", "submitted", "reviewing", "settlement_processing"].includes(
            request.status,
        ) &&
...
async function handleUnilateralTermination() {
    ...
    saving.value = true;
    try {
        const response = await adminPartnerApplicationService.terminate(
            application.value.id,
            {
        ...
        const request =
            (application.value?.termination_requests || []).find(
...
function unilateralNoticeDocument(request) {
    const row = [...(request?.documents || [])]
        .reverse()
        .find((item) =>
            ["unilateral_notice", "unilateral_termination_notice"].includes(
                item.document_type,
            ),
        );
...
function terminationFinalDocument(request) {
    const row = [...(request?.documents || [])]
        .reverse()
        .find((item) =>
            ["settlement_minutes", "final_termination_file"].includes(
                item.document_type,
            ),
        );
    const document = row?.generated_document;
...
function terminationFinalSignedBy(request, signerSide) {
    return (terminationFinalDocument(request)?.signatures || []).some(
        (signature) =>
            signature.signer_side === signerSide &&
...
function terminationFinalAdminSigned(request) {
    return (
        Boolean(request?.final_document_admin_signed_at) ||
        terminationFinalSignedBy(request, "sportgo")
    );
}

function terminationFinalOwnerSigned(request) {
    return (
        Boolean(request?.final_document_owner_signed_at) ||
        terminationFinalSignedBy(request, "owner")
    );
}
...
async function openTerminationFinalModal(request, generatedDocument = null) {
    const document = generatedDocument?.id
        ? {
              ...generatedDocument,
              title:
                  generatedDocument.title || "Biên bản chấm dứt hợp tác cuối",
...
async function signAndIssueUnilateralNotice() {
    ...
    try {
        const response =
            await adminPartnerApplicationService.signUnilateralNotice(
                unilateralNoticeModal.requestId,
                {
                    signature_image:
                        unilateralNoticeCanvas.value?.toDataURL("image/png") ||
                        "",
                    confirmation: true,
                },
            );
...
async function submitTerminationInlineAction(request) {
    ...
    try {
        const response =
            terminationInlineAction.type === "withdraw"
                ? await adminPartnerApplicationService.withdrawUnilateralNotice(
                      request.id,
                      { reason: terminationInlineAction.note },
                  )
                : await adminPartnerApplicationService.resolveUnilateralReconsideration(
                      request.id,
                      { note: terminationInlineAction.note },
                  );
        message.value = response.message;
...
async function handleConfirmTermination(request) {
    if (!confirm("Xác nhận đồng ý thanh lý hợp đồng theo yêu cầu của đối tác?"))
        return;
    clearAlerts();
    saving.value = true;
    try {
        const response =
            await adminPartnerApplicationService.confirmTermination(
                application.value.id,
                { termination_request_id: request.id },
            );
        message.value =
            response.message || "Đã xác nhận thanh lý và tạo quyết toán.";
        await loadApplication();
    } catch (err) {
...
function terminationNextAction(request) {
    if (
        ...
        return {
            ...
            description:
                "File mới là bản xem trước. Admin mở văn bản, ký trực tiếp và phát hành; trước đó cụm sân chưa bị khóa bởi công văn này.",
        };
    }
    if (
        isUnilateralNotice(request) &&
        ...
        return {
            ...
            description:
                "Công văn đã ký và gửi, cụm sân đã dừng nhận booking mới. Chủ sân cần đọc file và xác nhận tiếp nhận.",
        };
    }
    if (isTerminationDraftStatus(request.status)) {
        return {
            ...
            description:
                "Chủ sân đã tạo bản xem trước nhưng chưa ký OTP gửi chính thức. Admin chưa cần xử lý cho đến khi đơn được gửi.",
        };
    }
    if (canConfirmTerminationRequest(request)) {
        return {
            ...
            description:
                "Kiểm tra đơn chủ sân đã ký, lý do chấm dứt và nghĩa vụ liên quan rồi xác nhận để bắt đầu xử lý thanh lý.",
        };
    }
    if (isTerminationFutureBookingStatus(request.status)) {
        return {
            ...
            description:
                "Các booking chưa hoàn tất phải được hủy/hoàn tiền, phục vụ xong hoặc admin đánh dấu đã xử lý thủ công.",
        };
    }
    if (isTerminationSettlementStatus(request.status)) {
        return {
            ...
            description:
                "Nếu booking và công nợ đã xử lý ngoài hệ thống, admin xác nhận thủ công để sinh biên bản chấm dứt cuối.",
        };
    }
    if (
        isTerminationFinalSignatureStatus(request.status) &&
        !terminationFinalAdminSigned(request)
    ) {
        return {
            title: "Cần SportGo ký biên bản cuối",
            description:
                "SportGo ký bằng tài khoản admin hiện tại trước, sau đó chủ sân mới ký OTP xác nhận hoàn tất.",
        };
    }
    if (
        isTerminationFinalSignatureStatus(request.status) &&
        terminationFinalAdminSigned(request) &&
        !terminationFinalOwnerSigned(request)
    ) {
        return {
            title: "Chờ chủ sân ký biên bản cuối",
            description:
                "SportGo đã ký. Chủ sân cần vào màn hồ sơ chấm dứt để ký OTP phần bên B.",
        };
    }
    if (isTerminationTransitionStatus(request.status)) {
        return {
            ...
            description:
                "Hai bên đã ký biên bản cuối. Hệ thống sẽ thu hồi quyền chủ sân sau thời gian cấu hình.",
        };
    }
    if (isTerminationCompletedStatus(request.status)) {
        return {
            ...
            description:
                "Hợp đồng đã kết thúc, cụm sân chuyển sang trạng thái chấm dứt hợp tác.",
        };
    }
    if (isTerminationCancelledStatus(request.status)) {
        if (isUnilateralNotice(request)) {
            return {
                ...
                description:
                    "Cụm sân đã được mở lại nếu không còn khóa khác. File đã ký và lịch sử thu hồi vẫn được giữ để đối soát.",
            };
        }
        return {
            title: "Chủ sân đã hủy yêu cầu",
            description:
                "Yêu cầu dừng xử lý. Kiểm tra lịch sử nếu đã có thao tác thủ công phát sinh.",
        };
    }
    if (isTerminationRejectedStatus(request.status)) {
        return {
            ...
            description:
                "Admin đã từ chối yêu cầu chấm dứt. Kiểm tra lý do và lịch sử xử lý trước khi tạo yêu cầu mới.",
        };
    }
    return {
        title: "Theo dõi hồ sơ",
        description:
            "Kiểm tra booking, công nợ, văn bản và chữ ký để chuyển hồ sơ sang bước tiếp theo.",
    };
}
...
function terminationAdminSteps(request) {
    ...
    const bookingResolved =
        !Number(request.future_booking_count || 0) ||
        isTerminationSettlementStatus(status) ||
        isTerminationFinalSignatureStatus(status) ||
        isTerminationTransitionStatus(status) ||
        isTerminationCompletedStatus(status);
    const requestDone =
        !isTerminationDraftStatus(status) &&
        !isTerminationSubmittedStatus(status);

    if (isUnilateralNotice(request)) {
        const acknowledged = Boolean(
            request.workflow_state?.owner_acknowledged_at,
        );
        return [
            {
                key: "notice",
                label: "Ký công văn",
                state: isTerminationDraftStatus(status) ? "current" : "done",
            },
            {
                key: "ack",
                label: "Chủ sân xác nhận",
                state: isTerminationSubmittedStatus(status)
                    ? "current"
                    : acknowledged
                      ? "done"
                      : "pending",
            },
            {
                key: "booking",
                label: "Booking",
                state: isTerminationFutureBookingStatus(status)
                    ? "current"
                    : bookingResolved
                      ? "done"
                      : "pending",
            },
            {
                key: "settlement",
                label: "Đối soát",
                state: isTerminationSettlementStatus(status)
                    ? "current"
                    : isTerminationFinalSignatureStatus(status) ||
                        isTerminationTransitionStatus(status) ||
                        isTerminationCompletedStatus(status)
    ...
    return [
        ...
        },
        {
            ...
            state: ownerSigned
                ? "done"
                : adminSigned && isTerminationFinalSignatureStatus(status)
                  ? "current"
                  : "pending",
        },
    ];
...
async function handleMarkTerminationReady(request) {
    if (
        !confirm(
            "Xác nhận booking/công nợ đã xử lý thủ công và sinh biên bản chấm dứt cuối?",
        )
    )
        return;
    clearAlerts();
    ...
    try {
        const response =
            await adminPartnerApplicationService.markTerminationReady(
                request.id,
                {
        ...
        message.value =
            response.message ||
            "Đã xác nhận thủ công và sinh biên bản chấm dứt cuối.";
        await loadApplication();
...
async function handlePreviewFinalDocument(request) {
    ...
    try {
        const response =
            await adminPartnerApplicationService.previewTerminationFinalDocument(
                request.id,
            );
        ...
        error.value =
            err.message || "Không thể làm mới biên bản chấm dứt cuối.";
    } finally {
        saving.value = false;
    }
}
...
async function handleSignTerminationFinalDocument(request) {
    ...
    try {
        const response =
            await adminPartnerApplicationService.signTerminationFinalDocument(
                request.id,
                {
        ...
        message.value =
            response.message || "SportGo đã ký biên bản chấm dứt cuối.";
        terminationFinalSigning.accepted = false;
        await loadApplication();
...
async function handleManualResolveBooking(request, action) {
    ...
    try {
        const response =
            await adminPartnerApplicationService.manualResolveTerminationBooking(
                request.id,
                {
...
const InfoPanel = defineComponent({
    ...
    setup(props) {
        return () =>
            h("article", { class: "info-panel" }, [
                h("h3", props.title),
                h(
                    "dl",
                    (props.items || []).flatMap(([label, value]) => [
                        h("dt", label),
                        h("dd", value || "-"),
                    ]),
                ),
...
function identityLabel(value) {
    return (
        { cccd: "CCCD", cmnd: "CMND", passport: "Hộ chiếu" }[value] ||
        value ||
        "-"
    );
}
...
function documentTypeLabel(type) {
    if (type === "owner_termination_request")
        return "Đơn chủ sân yêu cầu chấm dứt";
    if (type === "termination_cancellation_request")
        return "Đơn chủ sân xác nhận hủy yêu cầu chấm dứt";
    if (type === "venue_scale_request")
        return "Đơn yêu cầu thay đổi quy mô sân";
    if (type === "venue_location_change_request")
        return "Đơn yêu cầu thay đổi vị trí cụm sân";
    if (type === "venue_scale_appendix") return "Phụ lục thay đổi quy mô sân";
    if (type === "venue_location_appendix")
        return "Phụ lục thay đổi vị trí cụm sân";
...
function generatedDocumentGroupMeta(document) {
    ...
    if (type === "venue_location_change_request") {
        return {
            ...
            label: "Đơn yêu cầu thay đổi vị trí",
        };
    }
    if (
        [
            "termination_request",
            "owner_termination_request",
...
function uploadedDocumentGroupMeta(document) {
    ...
    if (
        ["identity", "business_license", "facility", "bank", "lease"].includes(
            type,
        )
    ) {
    ...
    if (
        ["scale_request_documents", "scale_request_supplement"].includes(type)
    ) {
        return {
            key: "scale_request_uploads",
            label: "Hồ sơ yêu cầu thay đổi quy mô",
        };
    }
    if (
        ["location_change_documents", "location_change_supplement"].includes(
            type,
        )
    ) {
        return {
            key: "location_request_uploads",
            label: "Hồ sơ yêu cầu thay đổi vị trí",
        };
    }
...
function signatureSummary(signatures = []) {
    ...
    return (
        signatures
            ...
            .map(
                (signature) =>
                    `${signature.signer_side === "sportgo" ? "SportGo" : "Chủ sân"}: ${formatDate(signature.signed_at)}`,
            )
            .join(" · ") || "Chưa có chữ ký"
    );
}
...
function courtTypeName(court) {
    return (
        ...
        court?.court_type_name_snapshot ||
        "Loại sân"
    );
}

function coordinateText(item) {
    return item?.venue_latitude && item?.venue_longitude
        ? `${item.venue_latitude}, ${item.venue_longitude}`
        : "-";
}

function contactText(item) {
    return (
        [item?.venue_phone, item?.venue_email].filter(Boolean).join(" · ") ||
        "-"
    );
}
...
.admin-signing-context {
    border-left: 3px solid #16a34a;
    background: #f0fdf4;
    padding: 10px 12px;
}

.admin-signing-context strong {
    color: #166534;
    font-size: 13px;
}

.admin-signing-context p {
    color: #475569;
    font-size: 12px;
    line-height: 1.45;
}

.admin-signature-field {
    display: grid;
    gap: 7px;
}

.admin-signature-field > div {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 10px;
...
.termination-finance-grid .label,
.termination-finance-grid strong {
    display: block;
}
...
.termination-detail-disclosure {
    margin-top: 12px;
    border-top: 1px solid var(--admin-border, #e2e8f0);
}

.termination-detail-disclosure > summary {
    display: flex;
    min-height: 44px;
    align-items: center;
    justify-content: space-between;
...
/* Flat partner workspace: one surface per section, predictable reading order. */
.partner-detail-page {
    max-width: none;
    gap: 18px;
...
.partner-detail-page strong,
.partner-detail-page b,
.partner-detail-page h1,
.partner-detail-page h2,
.partner-detail-page h3,
.partner-detail-page h4,
.partner-detail-page h5,
.partner-detail-page button {
...
.partner-detail-page .title-block h2 {
    font-size: 26px;
    letter-spacing: -0.02em;
}

.partner-detail-page .title-block p,
.partner-detail-page .action-strip span,
.partner-detail-page .card-head span,
.partner-detail-page dt,
.partner-detail-page .muted,
.partner-detail-page .empty-text {
    color: #456455;
}

.partner-detail-page .notice,
.partner-detail-page .action-strip,
.partner-detail-page .review-panel,
.partner-detail-page .info-panel,
.partner-detail-page .content-card,
.partner-detail-page .partner-summary-rail {
    border: 1px solid var(--admin-border, #cfe0d5);
    border-radius: 6px;
    background: var(--admin-surface, #fff);
    box-shadow: none;
}
...
.summary-stat + .summary-stat {
    border-left: 1px solid var(--admin-border, #cfe0d5);
}
...
.partner-detail-page .status,
.partner-detail-page .badge,
.partner-detail-page .termination-progress span {
    display: inline-flex;
    min-height: 0;
    padding: 0;
    border: 0;
...
.partner-detail-page .status-rejected,
.partner-detail-page .status-cancelled,
.partner-detail-page .badge-rejected,
.partner-detail-page .badge-cancelled {
    color: #a33a32;
}
...
.partner-detail-page .tabs button {
    min-height: 42px;
    border: 0;
    border-radius: 0;
    border-left: 1px solid var(--admin-border, #cfe0d5);
...
.partner-detail-page .tabs button:first-child {
    border-left: 0;
}

.partner-detail-page .tabs button.active {
    background: var(--admin-primary-soft, #e5f5ea);
    color: var(--admin-success-text, #087a3d);
}

.partner-detail-page .review-more-menu,
.partner-detail-page .review-more-menu button,
.partner-detail-page .court-card,
.partner-detail-page .doc-row,
.partner-detail-page .signing-log-row,
.partner-detail-page .cluster-row,
.partner-detail-page .termination-card,
.partner-detail-page .termination-finance-grid > div,
.partner-detail-page .settlement-box,
.partner-detail-page .doc-group-head {
    border: 0;
    border-radius: 0;
    box-shadow: none;
    background: var(--admin-surface-muted, #f4faf6);
}
...
.partner-detail-page .review-more-menu button:hover {
    background: var(--admin-primary-soft, #e5f5ea);
}
...
.partner-detail-page .termination-progress {
    gap: 10px;
}

.partner-detail-page .termination-progress span + span {
    padding-left: 10px;
    border-left: 1px solid var(--admin-border, #cfe0d5) !important;
}
