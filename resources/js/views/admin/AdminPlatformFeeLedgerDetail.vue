        ...
        <div class="profile-section-card ledger-detail-main-content">
            <section class="detail-page">
                <button
                    ...
                    type="button"
                    @click="
                        $router.push({ name: 'admin-platform-fee-ledgers' })
                    "
                >
                    Quay lại danh sách
                </button>

                <div v-if="!ledger" class="panel empty">
                    Không tìm thấy kỳ phí.
                </div>
                <template v-else>
                    <!-- Ledger Info Bar -->
                    <div
                        class="ledger-info-bar"
                        style="
                            display: flex;
                            align-items: center;
                            gap: 12px;
                    ...
                    <section class="panel">
                        ...
                        <div class="info-grid">
                            ...
                            <div>
                                <span>Chủ sân</span
                                ><strong>{{ ledger.owner?.full_name }}</strong>
                            </div>
                            <div>
                            ...
                            <div>
                                <span>Bậc phí snapshot</span
                                ><strong>{{ ledger.tier_name }}</strong>
                            </div>
                            <div>
                                <span>Kỳ đóng</span
                                ><strong
                                    >{{ ledger.period_months }} tháng</strong
                                >
                            </div>
                            <div>
                                ...
                                ><strong
                                    ...
                                    {{ date(ledger.period_end) }}</strong
                                >
                            </div>
                            <div>
                                <span>Trạng thái hiệu lực</span
                                ><strong :class="ledger.period_warning_level">{{
                                    periodStatusLabel(ledger)
                                }}</strong>
                            </div>
                            <div>
                                <span>Khoảng sân snapshot</span
                                ><strong>{{ tierRangeLabel(ledger) }}</strong>
                            </div>
                            <div>
                                <span>Giá/sân/tháng snapshot</span
                                ><strong>{{
                                    money(ledger.price_per_court_month)
                                }}</strong>
                            </div>
                            <div>
                                <span>Giảm giá snapshot</span
                                ><strong>{{
                                    percent(ledger.discount_percent)
                                }}</strong>
                            </div>
                            <div>
                                <span>Thời điểm chốt giá</span
                                ><strong>{{
                                    dateTime(ledger.pricing_snapshotted_at)
                                }}</strong>
                            </div>
                            <div>
                                <span>Tổng trước giảm</span
                                ><strong>{{
                                    money(ledger.base_amount)
                                }}</strong>
                            </div>
                            <div>
                                <span>Số tiền giảm</span
                                ><strong>{{
                                    money(ledger.discount_amount)
                                }}</strong>
                            </div>
                            <div>
                    ...
                    <section class="panel">
                        ...
                        <div class="ledger-actions">
                            ...
                            <button
                                class="btn danger icon-text"
                                type="button"
                                :disabled="!canCancelLedger"
                                @click="cancelCurrentLedger"
                            >
                    ...
                    <section class="panel">
                        ...
                        <div v-if="ledger.receipt" class="receipt">
                            ...
                            <span
                                >{{ money(ledger.receipt.amount) }} -
                                {{ date(ledger.receipt.issued_at) }}</span
                            >
                    ...
                    <section class="panel">
                        ...
                        <div
                            ...
                            class="empty compact"
                        >
                            Chưa có email nhắc phí nào được gửi.
                        </div>
                        <div
                            v-else
                            class="table-scroll"
                            tabindex="0"
                            aria-label="Bảng lịch sử email nhắc thanh toán"
                        >
                            <table>
                                <thead>
                                    <tr>
                ...
                >
                    ...
                    >
                        ...
                        <p class="cancel-warning">
                            Chỉ kỳ chưa ghi nhận tiền mới được hủy. Dữ liệu kỳ
                            đã thanh toán sẽ không bị thay đổi.
                        </p>
                        <label>
                            Lý do hủy *
                            <textarea
...
export default {
    ...
    computed: {
        canCancelLedger() {
            return (
                this.ledger &&
                ["pending", "overdue"].includes(this.ledger.status) &&
                Number(this.ledger.amount_paid || 0) <= 0
            );
        },
    },
    methods: {
        ...
        async payFull() {
            await this.run(
                () =>
                    confirmLedgerPayment(this.ledger.id, {
                        amount: this.ledger.remaining_amount,
                    }),
        ...
        async submitActionDialog() {
            ...
            if (dialog.type === "overdue") {
                await this.run(
                    () => markLedgerOverdue(this.ledger.id, dialog.reason),
                    "Đã đánh dấu quá hạn.",
                );
        ...
        async unlockVenue() {
            await this.run(
                () => unlockVenueAfterPayment(this.ledger.id),
                "Đã mở khóa cụm sân.",
            );
        },
        ...
        async submitCancellation() {
            ...
            await this.run(
                () => cancelLedger(this.ledger.id, reason),
                "Đã hủy kỳ phí.",
            );
        },
        async sendCurrentReminder() {
            ...
            await this.run(
                () =>
                    sendPlatformFeeReminderEmail(this.ledger, type, {
                        force: true,
                    }),
