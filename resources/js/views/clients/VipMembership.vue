<template>
    <div class="w2-white-content">
        ...
        <header class="page-head">
            <div class="page-head-copy">
                ...
                <p class="page-subtitle">
                    Đặc quyền hoàn tiền sau mỗi lượt đặt sân, ưu đãi voucher và
                    ưu tiên hỗ trợ cho người chơi thể thao thường xuyên.
                </p>
            </div>
        </header>
        ...
        <div v-if="hasActiveSubscription" class="alert info" role="status">
            Bạn đang có gói VIP còn hiệu lực. Hệ thống hiện áp dụng 1 gói VIP
            tại một thời điểm cho mỗi tài khoản.
        </div>

        <!-- Current Active Plan Strip -->
        <section
            v-if="subscription"
            class="current-plan-strip"
            aria-label="Gói hiện tại"
        >
            <div class="current-plan-item">
                <span class="current-plan-label">Gói đang sử dụng:</span>
                <span class="current-plan-val">{{
                    subscription.package?.label || subscription.package?.name
                }}</span>
            </div>
            <div class="current-plan-item">
            ...
            <div class="current-plan-item">
                ...
                <span class="current-plan-val">{{
                    money(subscription.paid_amount)
                }}</span>
            </div>
        </section>
        ...
        >
            <div class="payment-intro">
                ...
                <span class="payment-amount">{{
                    money(paymentInfo.payment?.amount)
                }}</span>
                <p class="payment-hint">
                    Vui lòng quét mã QR hoặc chuyển khoản chính xác nội dung bên
                    dưới để hệ thống tự động kích hoạt gói.
                </p>
            </div>
                ...
                <div class="payment-details-list">
                    ...
                    <div class="payment-field">
                        ...
                        <span class="payment-field-val">{{
                            ...
                            paymentInfo.system_bank_account?.account_number ||
                            "-"
                        }}</span>
                        <button
                            type="button"
                    ...
                    <div class="payment-field">
                        <span class="payment-field-label"
            ...
            <div v-for="pkg in paidPackages" :key="pkg.id" class="plan-column">
                ...
                <div class="plan-cycles-block">
                    <div class="cycles-header">
                        ...
                        <span
                            v-if="cycleDiscount(pkg, 'yearly') > 0"
                            class="saving-note"
                        >
                            Tiết kiệm đến
                            {{
                                formatPercent(cycleDiscount(pkg, "yearly"))
                            }}%/năm
                        </span>
                    </div>
                    ...
                    <div class="cycle-buttons-stack">
                        ...
                        >
                            <div class="cycle-btn-left">
                                ...
                                <span
                                    v-if="cycleDiscount(pkg, cycle.key) > 0"
                                    class="cycle-discount-tag"
                                >
                                    Tiết kiệm
                                    {{
                                        formatPercent(
                                            cycleDiscount(pkg, cycle.key),
                                        )
                                    }}%
                                </span>
                            </div>
                            <div class="cycle-btn-right">
                                <span class="cycle-price-total">{{
                                    money(cycle.price)
                                }}</span>
                                <span class="cycle-price-breakdown"
                                    >{{
                                        money(cycleUnitPrice(cycle))
                                    }}/tháng</span
                                >
                            </div>
                        </button>
                ...
                <div class="plan-perks-block">
                    ...
                    <ul class="perks-list">
                        <li
                            ...
                            class="perk-row"
                        >
                            <span class="perk-check-glyph" aria-hidden="true"
                                >✓</span
                            >
                            <div class="perk-body">
                                <span class="perk-label">{{
                                    feature.value
                                }}</span>
                                <span class="perk-sub">{{
                                    feature.label
        ...
        >
            <section class="confirm-modal-card" role="dialog" aria-modal="true">
                ...
                <p class="confirm-note">
                    Sau khi quét mã thanh toán thành công, hệ thống sẽ tự động
                    kích hoạt gói VIP cho tài khoản của bạn.
                </p>
...
export default {
    ...
    methods: {
        ...
        packageDescription(pkg) {
            if (pkg.type === "saving")
                return "Tiết kiệm chi phí và tối ưu quyền lợi cho người chơi đều đặn.";
            if (pkg.type === "pro")
                return "Tối đa đặc quyền hoàn tiền, bài giao lưu và ưu đãi đặt sân.";
            ...
            return [
                {
                    ...
                    value:
                        cashback > 0
                            ? `Hoàn tiền ${this.formatPercent(cashback)}%`
                            : "Không có hoàn tiền",
                ...
                },
                {
                    ...
                    value:
                        voucherCount > 0
                            ? `${voucherCount} voucher VIP / tháng`
                            : "Không có voucher VIP",
                ...
                },
                {
                    ...
                    value:
                        voucherDiscount > 0
                            ? `Ưu đãi giảm ${this.formatPercent(voucherDiscount)}%`
                            : "Ưu đãi chuẩn",
                    label:
                        voucherDiscount > 0 && voucherMax > 0
                            ? `Tối đa ${this.money(voucherMax)} · đơn từ ${this.money(voucherMin)}`
                            : "Áp dụng theo chính sách gói",
                },
                {
                    key: "complaint",
                    value: pkg.priority_complaint
                        ? "Ưu tiên hỗ trợ & xử lý khiếu nại"
                        : "Hỗ trợ tiêu chuẩn",
