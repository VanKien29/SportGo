    ...
    <main class="detail-shell">
      ...
      <template v-else-if="item">
        ...
        <div class="detail-grid">
          <section class="detail-panel"><h2>Thông tin buổi giao lưu</h2><dl><div><dt>Sân</dt><dd>{{ item.booking.venue_name }}</dd></div><div><dt>Địa chỉ</dt><dd>{{ item.booking.venue_address || 'Chưa cập nhật' }}</dd></div><div><dt>Thời gian</dt><dd>{{ formatDate(item.booking.date) }} · {{ item.booking.start_time }} - {{ item.booking.end_time }}</dd></div></dl></section>
        ...
        <div class="detail-actions"><button v-if="item.can_leave" type="button" class="detail-button detail-button--danger" :disabled="leaving" @click="leave">{{ leaving ? 'Đang rút...' : 'Rút yêu cầu' }}</button><router-link class="detail-button" :to="{ name: 'ClientCommunityList' }">Quay lại cộng đồng</router-link></div>
      </template>
    </main>
  </div>
</template>
