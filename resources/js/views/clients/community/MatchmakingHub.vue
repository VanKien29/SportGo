        ...
        <div>
          ...
          <div v-else class="sg-matchmaking-list">
            <article v-for="post in posts" :key="post.id" class="sg3-card sg-matchmaking-card">
              ...
              <footer class="sg-matchmaking-card__foot"><span v-if="post.user_status" class="sg-matchmaking-request" :class="`is-${post.user_status}`"><AppIcon :name="post.user_status === 'approved' ? 'circleCheck' : 'clock'" :size="15" /> {{ requestLabel(post.user_status) }}</span><span v-else></span><div><router-link v-if="isOwn(post)" class="sg3-button sg3-button--secondary" :to="`/matchmaking-posts/${post.id}/manage`"><AppIcon name="users" :size="15" /> Xem người tham gia</router-link><button v-if="!isOwn(post) && post.user_status && post.user_status !== 'cancelled'" type="button" class="sg3-button sg3-button--secondary" :disabled="joining === post.id" @click="leave(post)"><AppIcon name="close" :size="15" /> Rút yêu cầu</button><button v-else-if="!isOwn(post)" type="button" class="sg3-button sg3-button--primary" :disabled="joining === post.id || (post.user_status && post.user_status !== 'cancelled') || post.status !== 'open'" @click="join(post)"><AppIcon name="users" :size="15" /> {{ joining === post.id ? 'Đang gửi...' : post.user_status === 'cancelled' ? 'Xin tham gia lại' : 'Xin tham gia' }}</button></div></footer>
            </article>
          </div>
        </div>
...
function requestLabel(status) { return { pending: 'Đang chờ chủ bài duyệt', approved: 'Đã được chấp nhận', rejected: 'Đã bị từ chối', cancelled: 'Đã rút yêu cầu' }[status] || status; }
