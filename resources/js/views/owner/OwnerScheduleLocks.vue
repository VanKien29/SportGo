    ...
    <div class="content-grid">
      <article class="form-card">
        ...
        <form @submit.prevent="createLock">
          ...
          <div class="selection-box" :class="{ empty: !selectedSlots.length }">
            <strong>{{ selectedSlots.length }} ô giờ trên {{ selectedCourtCount }} sân</strong>
            <button v-if="selectedSlots.length" type="button" @click="clearSelection">Bỏ chọn tất cả</button>
          </div>
          ...
          <div class="form-note">
            Các ô liền nhau trên cùng một sân sẽ được gộp thành một khoảng khóa. Nếu có ô bị trùng booking hoặc đã khóa, toàn bộ lần tạo sẽ dừng.
          </div>

          <button class="primary-btn" type="submit" :disabled="saving || !canSubmit">
            {{ saving ? 'Đang khóa...' : `Khóa ${selectedSlots.length} ô đã chọn` }}
          </button>
        </form>
      </article>
      ...
      <article class="schedule-card">
        ...
        <div v-else class="schedule-wrap">
          <div class="schedule-grid" :style="scheduleGridStyle">
            ...
            <div v-for="slot in activePeriodSlots" :key="slot.start_time" class="grid-head time-head">
...
export default {
  ...
  computed: {
    ...
    selectedCourtCount() {
      return new Set(this.selectedSlots.map((slot) => slot.venue_court_id)).size;
    },
    activePeriod() {
      return this.quickRanges.find((range) => range.key === this.activeTimePeriod)
        || this.quickRanges[0];
    },
    activePeriodSlots() {
      return this.scheduleSlots.filter((slot) => {
        const start = this.time(slot.start_time);
        return start >= this.activePeriod.start && start < this.activePeriod.end;
      });
    },
    scheduleGridStyle() {
      return {
  ...
  methods: {
    ...
    slotTitle(courtId, slot) {
      const status = this.statusFor(courtId, slot);
      if (this.isSelected(courtId, slot)) return `${this.time(slot.start_time)} - ${this.time(slot.end_time)} · Đã chọn`;
    ...
    pickSlot(court, slot) {
      ...
      const existingIndex = this.selectedSlots.findIndex((item) => item.key === key);
    ...
    buildSelectedRanges() {
      const grouped = [...this.selectedSlots]
        ...
        .reduce((result, slot) => {
          if (!result[slot.venue_court_id]) result[slot.venue_court_id] = [];
...
<style scoped>
.schedule-lock-page{display:grid;gap:18px;max-width:1320px}.page-head,.card-head{display:flex;justify-content:space-between;align-items:flex-start;gap:16px}.page-head h2,.card-head h3{margin:0;color:#0f172a}.page-head>div>p:last-child{margin:7px 0 0;color:#64748b}.eyebrow{margin:0 0 6px;color:#059669;font-size:11px;font-weight:900;letter-spacing:.1em}.content-grid{display:grid;grid-template-columns:330px minmax(0,1fr);gap:18px;align-items:start}.form-card,.schedule-card,.locks-card{border:1px solid #e2e8f0;border-radius:14px;background:#fff;box-shadow:0 8px 28px rgba(15,23,42,.04)}.form-card,.locks-card{padding:20px}.form-card form{display:grid;gap:15px;margin-top:18px}.form-card label{display:grid;gap:7px;color:#334155;font-size:13px;font-weight:850}.form-card input,.form-card select,.form-card textarea{width:100%;border:1px solid #cbd5e1;border-radius:9px;padding:10px 11px;background:#fff;color:#0f172a;font:inherit}.time-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.form-note{padding:11px;border-radius:9px;background:#f8fafc;color:#64748b;font-size:12px;line-height:1.5}.primary-btn,.secondary-btn,.danger-btn{border:0;border-radius:9px;padding:10px 14px;font:inherit;font-weight:850;cursor:pointer}.primary-btn{background:#059669;color:#fff}.secondary-btn{background:#f1f5f9;color:#334155}.danger-btn{background:#fee2e2;color:#991b1b}.primary-btn:disabled,.secondary-btn:disabled,.danger-btn:disabled{opacity:.55;cursor:not-allowed}.alert{padding:13px 15px;border-radius:10px;font-weight:800}.alert.error{background:#fee2e2;color:#991b1b}.alert.success{background:#dcfce7;color:#166534}.schedule-card{overflow:hidden}.schedule-headline{padding:18px 20px;border-bottom:1px solid #e2e8f0}.legend{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:12px;color:#64748b;font-size:11px;font-weight:800}.legend span{display:flex;align-items:center;gap:5px}.legend i{width:11px;height:11px;border-radius:3px;border:1px solid #cbd5e1}.legend .available{background:#fff}.legend .booking{background:#cbd5e1}.legend .holding{background:#fde68a}.legend .manual{background:#fca5a5}.state{padding:36px;text-align:center;color:#64748b}.schedule-wrap{max-width:100%;overflow-x:auto;border-top:1px solid #e2e8f0}.schedule-grid{display:grid;width:100%;min-width:760px}.grid-head,.court-cell,.slot-cell{min-height:48px;border-right:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0}.grid-head{display:grid;place-items:center;background:#f2f7ef;color:#334238;font-size:11px;font-weight:900}.time-head{padding:8px 4px;white-space:nowrap}.sticky-col{position:sticky;left:0;z-index:2}.grid-head.sticky-col{z-index:3}.court-cell{display:grid;align-content:center;gap:3px;padding:8px 10px;background:#fff}.court-cell strong{color:#0f172a;font-size:12px}.court-cell span{color:#64748b;font-size:11px}.slot-cell{min-width:0;padding:0;border-top:0;border-left:0;background:#fff;cursor:pointer}.slot-cell.available:hover{background:#d1fae5;box-shadow:inset 0 0 0 1px rgba(5,150,105,.35)}.slot-cell.booking{background:#cbd5e1}.slot-cell.holding{background:#fde68a}.slot-cell.manual{background:#fca5a5}.slot-cell:disabled{cursor:not-allowed}.locks-card .card-head{padding-bottom:16px;border-bottom:1px solid #e2e8f0}.count-badge{padding:5px 9px;border-radius:999px;background:#ecfdf5;color:#047857;font-size:12px;font-weight:850}.lock-list{display:grid}.lock-row{display:grid;grid-template-columns:210px 1fr auto;gap:16px;align-items:center;padding:14px 0;border-bottom:1px solid #e2e8f0}.lock-row:last-child{border-bottom:0}.lock-time{display:grid;gap:4px}.lock-time strong{color:#0f172a}.lock-time span,.lock-row p{margin:0;color:#64748b;font-size:13px}@media(max-width:1050px){.content-grid{grid-template-columns:1fr}.form-card form{grid-template-columns:repeat(2,1fr)}.form-note,.primary-btn,.form-card label:last-of-type{grid-column:1/-1}}@media(max-width:720px){.page-head,.schedule-headline{display:grid}.form-card form{grid-template-columns:1fr}.time-grid{grid-template-columns:1fr}.form-note,.primary-btn,.form-card label:last-of-type{grid-column:auto}.lock-row{grid-template-columns:1fr}.legend{justify-content:flex-start}}
</style>
<style scoped>
.selection-box{display:grid;gap:5px;padding:12px;border:1px solid #6ee7b7;border-radius:9px;background:#ecfdf5;color:#047857}.selection-box.empty{border-color:#cbd5e1;background:#f8fafc;color:#64748b}.selection-box span{font-size:12px;line-height:1.45}.selection-box button{justify-self:start;border:0;padding:0;background:transparent;color:#dc2626;font:inherit;font-size:12px;font-weight:850;cursor:pointer}.slot-cell.selected{background:#10b981;box-shadow:inset 0 0 0 2px #047857}.slot-cell.selected:hover{background:#059669}
.quick-ranges{display:flex;align-items:center;justify-content:flex-start;flex-wrap:wrap;gap:10px;padding:16px 20px;border-bottom:1px solid #e2e8f0;background:#fff}.quick-ranges button{appearance:none;width:152px;min-width:152px;height:50px;display:inline-flex;flex:0 0 152px;align-items:center;justify-content:center;gap:5px;margin:0;border:1px solid #d9e8d9;border-radius:9px;padding:0 14px;background:#fff;color:#64748b;font-family:inherit;line-height:1;white-space:nowrap;cursor:pointer;box-shadow:none;transition:background .16s ease,border-color .16s ease,color .16s ease,transform .16s ease,box-shadow .16s ease}.quick-ranges button strong{color:inherit;font-size:16px;font-weight:850;line-height:1}.quick-ranges button small{color:#7b8a80;font-size:12px;font-weight:700;line-height:1}.quick-ranges button:hover:not(:disabled):not(.active){border-color:#86efac;background:#f0fdf4;color:#4b6353;transform:translateY(-1px);box-shadow:0 4px 9px rgba(22,163,74,.08)}.quick-ranges button:hover:not(:disabled):not(.active) small{color:#718078}.quick-ranges button.active{border-color:#16a34a;background:#16a34a;color:#fff;box-shadow:0 5px 12px rgba(22,163,74,.18)}.quick-ranges button.active small{color:#647b6d}.quick-ranges button:disabled{opacity:.45;cursor:not-allowed}@media(max-width:720px){.quick-ranges{gap:8px;padding:14px}.quick-ranges button{width:auto;min-width:135px;height:46px;flex:1 1 135px;padding:0 12px}.quick-ranges button strong{font-size:15px}.quick-ranges button small{font-size:11px}}
</style>
