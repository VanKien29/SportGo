...
import { applyOwnerTheme } from '../../utils/ownerTheme.js';
...
export default {
  ...
  methods: {
    ...
    loadUserPresets() {
      const saved = localStorage.getItem('owner-user-presets');
      if (saved) {
        try {
    ...
    async fetchUiSettingsFromDb() {
      try {
        const data = await ownerUiSettingsService.getSettings();
        if (data) {
    ...
    async saveTheme() {
      ...
      applyOwnerTheme(settingsPayload);
...
.settings-container input.theme-name-input {
  flex: 1;
  height: 36px;
  padding: 0 12px;
  box-sizing: border-box;
...
.settings-container input.theme-name-input::placeholder {
  font-size: 12px !important;
  opacity: 0.65;
}
