...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <h1 class="text-3xl font-bold text-gray-900 mb-6">Chỉnh sửa Banner</h1>
            ...
            <div>
                ...
                @if ($banner->image_path)
                    <div class="mb-4">
                        ...
                        <img src="{{ Storage::url($banner->image_path) }}" alt="{{ $banner->title }}" class="max-h-32 rounded">
                    </div>
                @endif
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition">
                    ...
                    <label for="image" class="cursor-pointer">
                        <p class="text-gray-600">Click để upload ảnh mới</p>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" id="starts_at" name="starts_at" 
                        value="{{ old('starts_at', $banner->starts_at->format('Y-m-d\TH:i')) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label for="ends_at" class="block text-sm font-medium text-gray-700 mb-2">Ngày Kết Thúc *</label>
                    <input type="datetime-local" id="ends_at" name="ends_at" 
                        value="{{ old('ends_at', $banner->ends_at->format('Y-m-d\TH:i')) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
