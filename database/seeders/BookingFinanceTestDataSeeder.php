...
class BookingFinanceTestDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Live dashboard scenario: one court currently playing and one next booking.
        $liveNow = CarbonImmutable::now('Asia/Ho_Chi_Minh');
        $liveStart = $liveNow->subMinutes(15);
        $liveEnd = $liveNow->addMinutes(45);
        $nextStart = $liveEnd->addMinutes(15);
        $nextEnd = $nextStart->addHour();

        $this->booking([
            'booking_code' => 'BOOKING_LIVE_0001',
            'customer_id' => $customerNam->id,
            'venue_court_id' => $courts[0]->id,
            'requested_venue_court_id' => $courts[0]->id,
            'venue_cluster_id' => $cluster->id,
            'booking_date' => $liveNow->toDateString(),
            'start_time' => $liveStart->format('H:i:s'),
            'end_time' => $liveEnd->format('H:i:s'),
            'duration_minutes' => 60,
            'total_price' => 120000,
            ...
            'court_id' => $courts[0]->id,
            'start' => $liveStart->format('H:i:s'),
            'end' => $liveEnd->format('H:i:s'),
            'price' => 120000,
            'status' => 'active',
        ]]);

        $this->booking([
            'booking_code' => 'BOOKING_NEXT_0001',
            'customer_id' => $customerLinh->id,
            'venue_court_id' => $courts[1]->id,
            'requested_venue_court_id' => $courts[1]->id,
            'venue_cluster_id' => $cluster->id,
            ...
            'status' => 'confirmed',
            'created_by' => $staff->id,
            'created_at' => $liveNow->subHour(),
            'updated_at' => $liveNow,
        ], [[
            'court_id' => $courts[1]->id,
            'start' => $nextStart->format('H:i:s'),
            'end' => $nextEnd->format('H:i:s'),
            'price' => 120000,
            'status' => 'active',
        ]]);
