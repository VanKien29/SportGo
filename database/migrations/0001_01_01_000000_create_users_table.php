...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('status')->comment('Trạng thái tài khoản: pending_verify chờ xác thực, active được dùng, locked bị khóa, deactivated ngừng dùng.');
            $table->string('verification_channel')->comment('Kênh user chọn để nhận mã xác thực: email hiện làm trước, sms để phase sau.');
            $table->string('lock_type')->nullable()->comment('Kiểu khóa tài khoản: temporary theo thời hạn, permanent vĩnh viễn, auto do cấu hình tự động.');
